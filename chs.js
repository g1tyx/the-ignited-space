/*

 @name    : 锅巴汉化 - Web汉化插件
 @author  : 麦子、JAR、小蓝、好阳光的小锅巴
 @version : V0.6.1 - 2019-07-09
 @website : http://www.g8hh.com
 @idle games : http://www.gityx.com
 @QQ Group : 627141737

*/

//1.汉化杂项
var cnItems = {
    _OTHER_: [],

    //未分类：
    'Save': '保存',
    'Export': '导出',
    'Import': '导入',
    'Settings': '设置',
    'Achievements': '成就',
    'Statistics': '统计',
    'Changelog': '更新日志',
    'Hotkeys': '快捷键',
    'ALL': '全部',
    'Default': '默认',
    'AUTO': '自动',
    'default': '默认',
    "points": "点数",
    "Reset for +": "重置得到 + ",
    "Currently": "当前",
    "Effect": "效果",
    "Cost": "成本",
    "Goal:": "目标:",
    "Reward": "奖励",
    "Start": "开始",
    "Exit Early": "提前退出",
    "Finish": "完成",
    "Milestone Gotten!": "获得里程碑！",
    "Milestones": "里程碑",
    "Completed": "已完成",
    "Default Save": "默认存档",
    "Delete": "删除",
    "No": "否",
    "Saves": "存档",
    "Options": "选项",
    "Yes": "是",
    "Are you sure?": "你确定吗？",
    "Edit Name": "编辑名称",
    "Info": "信息",
    "Currently:": "当前:",
    "Appearance": "外观",
    "How the game looks.": "游戏看起来如何。",
    "Theme": "主题",
    "Show milestones": "显示里程碑",
    "Show TPS meter at the bottom-left corner of the page.": "在页面左下角显示 TPS。",
    "Show TPS": "显示 TPS",
    "None": "无",
    "Align modifier units": "对齐概览单位",
    "Align numbers to the beginning of the unit in modifier view.": "在概览视图中将数字与单元的开头对齐。",
    "Select which milestones to display based on criterias.": "根据标准选择要显示的里程碑。",
    "All": "全部",
    "Classic": "经典",
    "Configurable": "可配置",
    "Duplicate": "复制品",
    "About & Changelog": "关于 & 更新日志",
    "Charge": "充能",
    "Collapse": "折叠",
    "Craftable": "可制作",
    "Drill rock": "开采石头",
    "Drone Actions": "无人机动作",
    "Drone Upgrades": "无人机升级",
    "Export text": "导出文本",
    "Export to file": "导出到文件",
    "Game Data": "游戏数据",
    "Get export text": "获取导出文本",
    "None": "无",
    "Power up": "通电",
    "Hard reset": "硬重置",
    "Ignited Space": "激燃太空",
    "Import from file": "从文件导入",
    "Import from text": "从文本导入",
    "Landing zone": "着陆区",
    "Planet": "行星",
    "Power": "电力",
    "Repair Drill": "修理钻头",
    "Select file": "选择文件",
    "Stone": "石头",
    "Time:": "时间：",
    "Wipe game data": "擦除游戏数据",
    "You need to enable JavaScript to run this app.": "您需要启用 JavaScript 才能运行此应用程序。",
    "Consume": "消耗",
    "Drill rock at cost of some energy": "开采石头，会消耗一些电力",
    "Drill ore at cost of some energy": "开采矿石，会消耗一些电力",
    "Never": "从不",
    "Produce": "生产",
    "Recover your energy power": "恢复你的电力",
    "Ore": "矿石",
    "Drill ore": "开采矿石",
    "Allows building quarry": "允许建造采石场",
    "Allows digging and picking more materials": "允许挖掘和捡起更多材料",
    "Auto-quarrying": "自动开采石头",
    "Repair Charging Cable": "修理充电线",
    "Repair Manipulators": "修理机械手",
    "Allows creating electricity power by burning biomass": "允许通过燃烧生物质来产生电力",
    "Automatically collect some biomass": "自动收集一些生物质",
    "Bacteria Incubator": "细菌培养箱",
    "Biomass": "生物质",
    "Collect Biomass": "收集生物质",
    "Combustion": "燃烧",
    "Dig and get some organics": "挖掘并获得一些有机物",
    "Drill some stone": "开采一些石头",
    "Further facilities repair": "进一步的设施修理",
    "Industrial Buildings": "工业建筑",
    "Repair Charging Cable": "修理充电线",
    "Repair Drill": "修理钻头",
    "Automatically collect some stone": "自动收集一些石头",
    "Quarry": "采石场",
    "Repair machinery": "修理机器",
    "Repair machinery. Improves your automatic quarries and mines by 50%": "修理机器。采石场和矿井的产量提升 50%",
    "Auto-miners": "自动采矿",
    "Allows building mine": "允许建造矿山",
    "Collect some organics": "收集一些有机物",
    "Repair storage module": "修理存储模块",
    "Autonomous power supply": "自动化电力供应",
    "Repair storage module. Allows building storage sections": "修理存储模块。允许建造存储区域",
    "Bacteria Incubator": "细菌培养箱",
    "Thermoelectric Power plant": "热电厂",
    "Expand": "展开",
    "Burns biomass to obtain electricity": "燃烧生物质以获取电力",
    "Power Buildings": "电力建筑",
    "Automatically collect some ore": "自动收集一些矿石",
    "Mine": "矿井",
    "Storage section": "存储区域",
    "Storage facilities": "存储设施",
    "Combustion cameras": "燃烧室",
    "Storage Buildings": "存储建筑",
    "Decrease biomass consumption by thermoelectric power plants by 25%": "热电厂的生物质消耗量减少 25%",
    "Metal": "金属",
    "Provides additional storage space": "提供额外的存储空间",
    "Storage": "存储",
    "Wood": "木头",
    "Boost power supply": "提升电力供应",
    "Repair battery": "修理电池",
    "Bio technology": "生物技术",
    "Increase incubators output by 25%": "细菌培养箱产量提升 25%",
    "Increase quarries output by 25%": "采石场产量提升 25%",
    "Quarry technology": "采石技术",
    "Bacteria selection": "菌种筛选",
    "Ore production": "矿石生产",
    "Power capacity": "电力容量",
    "Increase electricity consumption by incubators, but incubators start producing oxygen": "细菌培养箱的耗电量增加，但同时能制造氧气",
    "Combustion technology": "燃烧技术",
    "Increase thermal plant output by 25%": "热电厂的电力产量提升 25%",
    "Economy Boost": "经济提升",
    "Increase mine output by 25%": "矿井产量提升 25%",
    "Mine technology": "矿山技术",
    "More Storage": "更多存储空间",
    "Breath In": "吸气",
    "Storage hub": "存储中心",
    "Provides more storage space, but consumes power": "提供更多存储空间，但需要消耗电力",
    "Improved thermal plants": "改良热电厂",
    "Allow building oxygen compressors to store more oxygen": "允许建造氧气压缩机以储存更多的氧气",
    "Basic habitability requirements": "基本宜居性要求",
    "Oxidase burning cameras": "氧化酶燃烧室",
    "Oxygen": "氧气",
    "Oxygen compressor": "氧气压缩机",
    "Oxygen compressors": "氧气压缩机",
    "Thermal plants are 50% more efficient, but now consume oxygen": "热电厂的效率提升 50%，但会消耗氧气",
    "Water pump": "水泵",
    "Water storage": "水存储设施",
    "Allow building water pumping stations": "允许建造水泵",
    "Increase stone, ore and biomass storage from storage section and hub by 25%": "存储区域和存储中心的石头、矿石和生物质上限提升 25%",
    "Reserve batteries": "备用电池",
    "Storage Bonuses": "存储加成",
    "Storage technology": "存储技术",
    "Thermoelectric plants now also provide small power storage": "热电厂现在也提供少量电力上限",
    "Water pumping": "抽水",
    "Water": "水",
    "Provides water storage": "提供水资源的存储功能",
    "Ship hold": "船舱",
    "Provides water supply": "生产水资源",
    "To max:": "距离满仓：",
    "To empty:": "距离耗尽：",
    "Breakdown:": "明细：",
    "Base battery": "基础电池",
    "Base": "基础",
    "Provides more storage for oxygen": "提供更多的氧气存储",
    "Allows building battery. Also, improves power output by 20%": "允许建造电池。此外，电力产量提升 20%",
    "Electrolytes": "电解质",
    "Final preparation": "最后的准备",
    "Income Bonuses": "产量加成",
    "Nutrition": "营养物质",
    "Repair supply module": "修理生命维持模块",
    "Battery": "电池",
    "Provides power storage": "提供电力存储",
    "Start generating trace amount of nutrition": "开始产生微量营养物质",
    "Enables colonists arrival": "使殖民者能够到达",
    "First colonists": "第一批殖民者",
    "Living Cabin": "生活舱",
    "Repair docking module": "修复对接模块",
    "You start with broken ship on hostile planet. Your main goal is to prepare for firts colonists to arrive, and build prosperous civilisation": "你在一颗环境恶劣的行星上，开始只有一艘损坏的飞船。你的主要目标是为第一批殖民者的到来做好准备，并逐步建立繁荣的文明",
    "Empty and cold here": "这里空旷寒冷",
    "This game was pretty much inspired by": "这个游戏的灵感来自于",
    "Popups will now appear always to right": "弹出窗口将会出现在画面右侧",
    "Basic game mechanics: buildings, researches, upgrades, colony management": "基本游戏机制：建筑、研究、升级、殖民地管理",
    "Cargo Sorting": "货物分拣",
    "Colonists": "殖民者",
    "Colony": "殖民地",
    "Provides living space for 2 colonists": "为2名殖民者提供生活空间",
    "Repair research module": "修复研究模块",
    "Repair research module to enable researchers working at it and produce some science.": "修复研究模块，使研究人员能够在其中工作，并产出一些科学成果。",
    "Repair sorting machinery. Gives 75% bonus to stone, ore and biomass storage.": "维修分拣机械。为石头、矿石和生物质储存提供 75% 的加成。",
    "Research": "研究",
    "Residental Buildings": "住宅楼",
    "Aesthetics": "美学",
    "Basic": "基础",
    "Exo farming": "外来农业",
    "Free colonists:": "自由殖民者：",
    "Generates research": "产生研究",
    "Happiness affects your colonists productivity. Has no effect on machinery production or crafting": "幸福会影响您的殖民者的生产力。对建筑产量和工艺产量没有影响",
    "Jobs": "工作",
    "Learn how to work with soil on a foreign planet. Allows you hiring farmers": "了解如何在外星球上处理土壤。 允许你雇佣农民",
    "Miner": "矿工",
    "Mines ore": "开采矿石",
    "Mines stone": "矿石",
    "Overview": "概览",
    "Population": "人口",
    "Population:": "人口：",
    "Quarrymen": "采石工人",
    "Research: Exo farming": "研究：外来农业",
    "Researcher": "研究员",
    "Researches": "研究",
    "Science": "科学",
    "Scientific basics": "基础科学",
    "Farmer": "农民",
    "Farming tools": "农具",
    "Generates nutrition": "产生营养",
    "Improves farming output by 10%.": "农业产出提升 10%。",
    "Learn more about minerals available at your new home": "更深入地了解您新家园所拥有的矿物质",
    "Learn your atmosphere closer. Unlocks new upgrades and buildings": "更深入地了解你的大气层。解锁新的升级和建筑",
    "Meteorology": "气象学",
    "Mineralogy": "矿物学",
    "Service Industry": "服务业",
    "Unlock providing some services to your groving colony": "解锁为您的殖民地提供一些服务的功能",
    "Exo biology": "外星生物学",
    "Learn more about local organisms": "更深入地了解这里的生物",
    "Bio laboratory": "生物实验室",
    "Building cranes": "建筑起重机",
    "Clinic": "诊所",
    "Decrease building level prices scaling by 10%.": "将建筑升级的价格提升比例降低 10%。",
    "Deep mining": "深井采矿",
    "Entertainment": "娱乐",
    "Exo-forestry": "外星林学",
    "Fertilizing": "施肥",
    "Healthcare": "卫生保健",
    "Improves stone and ore output by 20%.": "石头和矿石产量提升 20%。",
    "Improves water output by 20%.": "将出水量提升 20%。",
    "Improves your stone, ore and biomass storage facilities by 50%.": "石头、矿石和生物质上限提升 50%。",
    "Learn how to work with exotic ores. Unlocks crafting": "了解如何处理这颗星球上的独特矿石。解锁工艺生产",
    "Lets learn how to plant something bigger in this empty grounds": "让我们学习如何在这片空地上种更大的东西",
    "Metallurgy": "冶金",
    "Meteorology Station": "气象站",
    "Monument": "纪念碑",
    "Nutritious Flora": "富营养菌群",
    "Place some special flora to your incubators to make them 50% more productive.": "在您的细菌培养箱中放置一些特殊的菌群，使其产量提高 50%。",
    "Platinum": "铂金",
    "Positive Effect": "正面作用",
    "Provides 25 science storage and boost science output by 2% multiplicative": "提供 25 研究点上限，并乘算提升 2% 研究点输出",
    "Provides basic healthcare service": "提供基本的医疗服务",
    "Provides even more science storage and boost science output by 5% multiplicative": "提供更多的研究点上限，并将研究点产出提高 5%",
    "Provides some aesthetics": "提供一些美学",
    "Public Buildings": "公共建筑",
    "Science Buildings": "科学建筑",
    "Sociology": "社会学",
    "Underground vaults": "地下金库",
    "Unlocks new ways to make your little society happier and more efficient": "解锁让您的小社会更快乐、更高效的新方法",
    "Water deposits": "积水",
    "Your farmers start consume biomass, but increase nutrition output.": "您的农民将会消耗生物质，但会增加营养物质输出。",
    "Beam": "木梁",
    "Better cables": "优质电缆",
    "Brick": "砖头",
    "Cargo cranes": "货运起重机",
    "Chemistry": "化学",
    "Converts ore and wood into rolled metal": "将矿石和木材转化为轧制金属",
    "Crafting": "制造",
    "Forest Plant": "森林",
    "Forest plants now boost biomass production": "森林将会提升生物质产量",
    "Forester": "林务员",
    "Forester tools": "林务员工具",
    "Fruit trees": "果树",
    "Further research precious minerals at your new habitat": "进一步研究您新家园的珍贵矿物",
    "Generates oxygen": "产生氧气",
    "Geology": "地质学",
    "Improves all resources storages.": "提升所有资源上限。",
    "Improves foresters output.": "提升林务员产量。",
    "Improves stone and ore output.": "提升石头和矿石的产量。",
    "Improves wood output by foresters.": "提升林务员的木材产量。",
    "Improves your power capacity and output.": "提升您的电力上限和产量。",
    "Metal containers": "金属制容器",
    "Metal machine parts": "金属制机械零件",
    "Metal tools": "金属制工具",
    "Natural Environment": "自然环境",
    "Park zone": "公园区域",
    "Parking zone provides basic possibility to relax for your people": "公园区域为您的人民提供基本的放松机会",
    "Plate": "金属板",
    "Provides more raw materials storage": "提供更多的基础材料上限",
    "Provides oxygen and water storage": "提供氧气和水存储",
    "Smelter": "冶炼厂",
    "Tank": "水箱",
    "Tree care": "树木护理",
    "Tree plants also provide nutrition.": "森林将会产出营养物质。",
    "Unlocks new materials and upgrades": "解锁新材料和升级",
    "Warehouse": "仓库",
    "Artisan": "工匠",
    "Crafting Slots:": "制造中：",
    "Crafts various goods": "制造各种工艺材料",
    "Free slots:": "空闲：",
    "Plants and cuts wood": "种植和砍伐木材",
    "Space": "太空",
    "Support project": "支持本游戏",
    ", but it's not a clone.": "，但并非一款和它完全一样的游戏。",
    "Added highlight on missing resource when hower over building/upgrade/research": "在建造/升级/研究处添加了对所缺少资源的突出显示",
    "Added manual save and hard reset buttons": "增加手动保存和硬重置按钮",
    "Added new early space content (researches, buildings)": "增加新的早期太空内容（研究、建筑）",
    "Added prestige mechanics": "增加重置机制",
    "Approximately 48 hours of playable content": "大约 48 小时的可游玩内容",
    "By Strange Matter Gaming": "作者：Strange Matter Gaming",
    "Changed size and background for buildable/upgradeable items to make them easier to see/click": "更改了可建造/可升级物品的大小和背景，使其更易于查看/点击",
    "Fixed bug when max colonists amount could be not integer value": "修复了最大殖民者数量可能不是整数的错误",
    "Join our Discord": "加入我们的 Discord 频道",
    "Kittens game": "猫国建设者（Kittens game）",
    "Negative and capped resources are now highlighted": "净产量为负的资源和达到上限的资源现在突出显示",
    "Support on Patreon": "在 Patreon 上支持作者",
    "v0.0.1 First Playable Prototype": "v0.0.1 首个可玩的原型版本",
    "v0.0.1a UI and small features update": "v0.0.1a UI 和小功能更新",
    "v0.0.2 Prestige mechanics & New content": "v0.0.2 重置机制和新内容",
    "Administrative service center": "行政服务中心",
    "Advanced anatomy": "高级解剖学",
    "Allow": "允许",
    "Apartment": "公寓",
    "Artificial organs": "人造器官",
    "Better understanding of quantum physics can significantly boost your civilization": "深入地研究量子物理学可以让你的文明更进一步",
    "Bigger grants": "大幅资助",
    "Building materials factory": "建筑材料工厂",
    "Cinema": "电影院",
    "Clinics are working in regular way, providing service using only reliable traditional medicine methods": "诊所以正常的方式运作，只使用可靠的传统医学方法提供服务",
    "Computer.": "电脑",
    "Cottage": "小屋",
    "Data Center": "数据中心",
    "Each level decrease public services requirements by your people by 2%": "每级令您人民的公共服务需求降低 2%",
    "Economy": "经济学",
    "Electronics factory": "电子元件工厂",
    "Experimental medicine": "实验性医学",
    "Forbid": "禁止",
    "Forest plants operating as normal": "林业以正常的方式运作",
    "Forest plants produce 50% of oxygen only, but provide small bonus to \"Aesthetics\"": "森林只产生 50% 的氧气，但会提供少量的美学加成",
    "Forest plants produce 50% of oxygen only, but provide small bonus to \"Entertainment\"": "森林只产生 50% 的氧气，但会提供少量的娱乐加成",
    "Further research human body can make maintaining larger colony much easier": "对人体的进一步研究令人口的进一步扩张成为可能",
    "Great building devoted to the Gods. Significantly boosts your religion service": "献给众神的伟大建筑。提供显著的宗教加成",
    "Great Radio Transmitter": "大型无线电发射机",
    "Increase healthcare service but decrease religious": "提供医疗保健加成，但减少宗教加成",
    "Infinity": "无限",
    "Its too little space on our planet, isn't?": "我们星球上的空间太小了，不是吗？",
    "Knowledge": "知识",
    "Law Water Filtering Control: On": "水过滤控制：开",
    "Laws & Order": "法律与秩序",
    "Metallurgy Complex": "冶金综合体",
    "Nature Reserve": "自然保护区",
    "Negative Effect": "负面影响",
    "No effect": "无影响",
    "No grants": "无资助",
    "Normal": "普通",
    "Off": "关闭",
    "On": "开启",
    "Picnic zones": "野餐区域",
    "Plastics": "塑料",
    "Policeman": "警察",
    "Policemen consume 0.5 Nutrition and 0.2 water each but provides better service": "警察每人将会消耗 0.5 单位营养物质和 0.2 单位水，但能提供更高的安保加成",
    "Policemen does not consume additional resources": "警察不消耗额外资源",
    "Provides 5% bonus to your materials storage in warehouses": "仓库的上限提升 5%",
    "Provides additional 2% boost to your colonists production": "殖民者的产量提升 2%",
    "Provides additional living space for 2 colonists": "提供足以容纳 2 名殖民者的额外生活空间",
    "Provides additional living space for 5 colonists": "提供足以容纳 5 名殖民者的额外生活空间",
    "Provides additional science capacity and generation": "提供额外的研究点上限和产量",
    "Provides bonus to bricks, beams and plates production": "提升砖块、木梁和金属板的产量",
    "Provides bonus to metal output": "提升金属的产量",
    "Provides bonus to wires, semiconductors and computers production": "提升电线、半导体和计算机的产量",
    "Provides power from solar radiation": "将太阳的辐射转化为电力",
    "Provides some religious service": "提供少量宗教加成",
    "Provides some security": "提供一定量的安保加成",
    "Quantum Physics": "量子物理学",
    "Religion": "宗教",
    "Research grants": "研究资助",
    "Rocket science": "火箭科学",
    "Rubber": "橡胶",
    "Rules of behavior in the forest": "森林行为准则",
    "Science buildings consume 150% more electricity but provide 10% more science capacity": "地面科学建筑额外消耗 150% 的电力，但提供额外 10% 的研究点上限",
    "Science buildings consume 50% more electricity but provide 5% more science capacity": "地面科学建筑额外消耗 50% 的电力，但提供额外 5% 的研究点上限",
    "Science buildings operate as normally": "地面科学建筑正常运作",
    "Security": "安保",
    "Security dotations": "安保资助",
    "Semiconduct.": "半导体",
    "Setting research as priority of your civilization can boost your development, but it won't be free.": "将科研作为您文明的最高优先级可以加速发展，但为此也需要付出一些代价。",
    "Shrine": "圣地",
    "Small grants": "小额资助",
    "Social": "社交",
    "Social research center": "社会研究中心",
    "Solar Panel": "太阳能电池板",
    "Storage container": "存储容器",
    "Temple": "寺庙",
    "Using 3D printers in creating or healing organs can significantly boost healthcare but religious people can suffer": "使用 3D 打印机制造或修复人体器官可以显著提高医疗加成，但宗教人士可能会因此感到不适",
    "Using experimental medicine reduces healthcare efficiency of clinics, but they now provide some max research boost": "使用实验性医学会降低诊所的医疗加成，但能令其提供少量的研究点上限",
    "Water Filtering Control": "水过滤控制",
    "Waterpumps consume 4 times more power. Increase healthcare by 10.": "水泵会消耗 4 倍的电力，医疗加成增加 10。",
    "Waterpumps working in regular way": "水泵正常工作",
    "Wire": "金属丝",
    "Your forest plants can be used in different modes, providing various advantages to you.": "您可以用不同的方式利用森林，来为您提供各种加成。",
    "Your police starts consuming more resources but provide better service.": "您的警察将会消耗更多资源，但能提供更高的安保加成。",
    "3D Printers construction": "3D 打印建造",
    "Advanced metallurgy": "高级冶金学",
    "Advanced Physics": "高等物理学",
    "Advanced Science": "高级科学",
    "Advanced water filtering": "高级水过滤",
    "Allows you building containers that providing boost to your storage capacity": "允许您建造能提高仓库上限的存储容器",
    "Chemical analysis": "化学分析",
    "Composite metals": "复合金属",
    "Computers": "电脑",
    "Electricity Study": "电力研究",
    "Energy Saving": "节能技术",
    "Further decrease building cost": "进一步降低建筑成本",
    "Further increase power supply": "进一步增加电力产量",
    "Gain 5% bonus to all crafting activities": "所有工艺生产的产量提升 5%",
    "Increase policemen efficiency by 25%": "警察的安保加成提高 25%",
    "Increase smelter output": "提升冶炼厂的产量",
    "Industry Automation": "工业自动化",
    "Multistore Warehouses": "多层式仓库",
    "Organs printing": "器官打印",
    "Platinum drills": "铂金钻头",
    "Platinum tools": "铂金工具",
    "Platinum tools provides additional production boost to your wood and crafting beams, plates and bricks": "铂金工具能提升您的木材、木梁、砖块和金属板的产量",
    "Rubber Details": "橡胶细节",
    "Sealing": "密封",
    "Significantly reduces building power cost": "显著降低建造和升级建筑的电力成本",
    "Surveillance Cameras": "监控摄像头",
    "Unlocks computers crafting and building data-centers": "解锁计算机制造和数据中心的建造",
    "Unlocks new medicine mode order": "解锁新的医药模式法令",
    "Upgrade wiring": "优化布线",
    "Using composite metals allows to increase metal crafting output": "使用复合金属来增加金属相关工艺的产量",
    "Using new materials further decreasing building costs scale": "使用新材料进一步降低建筑升级成本的增长率",
    "Your warehouses can now store twice as much goods": "您的仓库现在可以存储两倍数量的货物",
    "Fuel": "燃料",
    "Geography": "地理学",
    "Geography knowledge unlocks expeditions": "对地理学的研究可以解锁探险功能",
    "Orbit Buildings": "行星轨道建筑",
    "Provides power from solar radiation. More efficient comparing to planet surface analog": "将太阳辐射转化成电力。和它的地面版本相比效率更高",
    "Rocket": "火箭",
    "Space Constructions": "空间构造",
    "": "",
    "Solar Panel Satellite": "太阳能电池板卫星",
    "Allow building cottages.": "允许建造别墅。",
    "Civilized life": "文明生活",
    "Cottages": "小屋",
    "Improved electrolytes": "改善电解质",
    "Improves power capacity.": "提高电源容量。",
    "Improves water output.": "改善水的输出。",
    "Law": "法律",
    "Miners can now mine platinum.": "矿工现在可以开采铂金。",
    "Platinum mining": "铂金开采",
    "Unlocks new scientific building": "解锁新的科学建筑",
    "Allows further advance of your science": "进一步推进你的科学发展",
    "Allows using more saving rocket engine techs, which reduces rockets consumption by 20% for all space structures and ships": "允许使用更节省燃料的火箭发动机技术，从而将所有太空建筑和飞船的火箭消耗减少 20%",
    "Astrophysics": "天体物理学",
    "Better Photo-elements": "改良光电元件",
    "Boosts computers production": "提高计算机产量",
    "Boosts knowledge about virus diseases. Each level decrease colony health consumption by 5%": "加强对病毒性疾病的研究。每级减少殖民者医疗保健消耗 5%",
    "Boosts your solar satellites and solar panels output by 20%": "将您的太阳能卫星和太阳能电池板输出提高 20%",
    "Complete": "完成",
    "Conductive nano-particles": "导电纳米粒子",
    "Expedition": "探险",
    "Expedition Center": "探险中心",
    "Expeditions completed:": "完成的探险：",
    "Faster Rockets": "高速火箭",
    "Fuel Factory": "燃料工厂",
    "Genetic modification of organisms can help boost nutrition and biomass production": "生物体的基因改造可以提高营养物质和生物质产量",
    "Ice Melting Station": "融冰站",
    "Improved Lithography": "改进光刻机",
    "Improves your batteries and accumulators": "改进您的电池和蓄电池",
    "Laser Cutting": "激光切割",
    "Laser Cutting of metal parts significantly simplifies plates and wires production": "金属零件的激光切割显著简化了金属板和电线的生产",
    "Last Loot": "最后战利品",
    "Loot multiplier: x": "战利品乘数：x",
    "Melts ice on your cold satellite to obtain water": "融化冰冷卫星上的冰，以获得水",
    "Moon Buildings": "月球建筑",
    "Move toxic fuel production outside of your planet to avoid ecology restrictions and enlarge your fuel production capabilities": "将有毒的燃料生产转移到你的星球之外，来规避生态限制，并扩大燃料生产能力",
    "Nanobots": "纳米机器人",
    "Orbital Tourism Center": "轨道旅游中心",
    "Pandemic research center": "流行病研究中心",
    "Prestige": "重置",
    "Provides 5% multiplicative bonus to your expeditions outcome. Consumes some oxygen that your expeditions will take with them.": "为您的探险提供 5% 的加成。需要消耗一些氧气，以供探险队使用。",
    "Reducing time of flight allows to make your space tourism 50% more efficient": "减少飞行时间，可以使您的太空旅行效率提高 50%",
    "Rocket Flight Technology": "火箭飞行技术",
    "Small space station that provides some entertainment and aesthetics to your people": "为您的人民提供娱乐和美学加成的小型空间站",
    "Unlocks building nanobots factory on your satellite that provides bonus to all crafting activities": "解锁在您的卫星上建造纳米机器人工厂的能力，它将为所有工艺生产提供产量加成",
    "Conductive Nano-particles": "导电纳米粒子",
    "Deep Mining": "深井采矿",
    "Farming Tools": "农具",
    "Forester Tools": "林务员工具",
    "Fuel Conservation": "节省燃料",
    "GMO": "转基因生物",
    "Improved fuel combustion cycle allows to install less rockets to achieve the same thrust. Reduces rocket consumption by 20% for all space structures and ships": "改进的燃烧循环令更少的火箭也能输出相同的推力。所有空间建筑和飞船的火箭消耗减少 20%",
    "Improved photoelement technology allows you to boost solar satellites and solar panels output by 20%": "改良的光电元件技术将使太阳能卫星和太阳能电池板的电力产量提高 20%",
    "Improved smelting technique increases smelter output": "改进的冶炼技术将提高冶炼厂的产量",
    "Improved Storage": "改良存储",
    "Lay new wires to further increase power supply": "铺设新的电线，进一步增加电力产量",
    "Metal Machine Parts": "金属机械零件",
    "Metal Tools": "金属工具",
    "New methods of lithography improve assembling processes. Boosts computers production": "新的光刻工艺改进了计算机的装配流程。提高计算机产量",
    "Rangers": "护林员",
    "Repair Battery": "修复电池",
    "Repair Supply Module": "修复补给模块",
    "Underground Vaults": "地下金库",
    "Unlocks nanobots factory building on your satellite that provides bonus to all crafting activities": "解锁在您的卫星上建造纳米机器人工厂的能力，它将为所有工艺生产提供产量加成",
    "Using improved flight path calculation reduces total flight time. Space tourism is now 50% more efficient": "使用改进的飞行路径计算方式，减少飞行时间。太空旅游的娱乐和美学加成提升 50%",
    "Using lasers to cut out metal parts significantly simplifies plates and wires production": "使用激光切割金属零件，显著提升金属板和线材的产量",
    "Using new conductive particles improves the efficiency of your batteries and accumulators": "使用新的导电粒子，提高电池和蓄电池的电力上限",
    "Water Deposits": "积水",
    "Added \"Expeditions\" - active way to speed up getting resources": "添加了“探险”，一种主动获取资源的方式",
    "Available actions are now highlighted on hover": "可用的操作现在在悬停时突出显示",
    "Controls are shown disabled now when they not available": "控件在不可用时显示为已禁用",
    "Edited upgrade descriptions": "优化部分升级的说明",
    "Expedition center now also requires Geography research": "探险中心需要前置地理学研究",
    "Fixed bug when locked resources still were displaying in bonuses or requirements list": "修复了未解锁的资源仍然显示在奖励或需求列表中的错误",
    "Fixed bug when not active expedition centers provided boost to expedition multiplier": "修复了未开启的探险中心依然提供探险乘数提升的错误",
    "Fixed descriptions of some upgrades": "修复了一些升级的描述",
    "Inverted colors for in-progress items": "正在进行中的项目会反色显示",
    "Items that will never be available now highlighted in different color": "以目前状态无论等待多久都不可用的项目现在以不同颜色突出显示",
    "v0.0.2a Small UI updates": "v0.0.2a 小规模 UI 更新",
    "v0.0.2b Expeditions added": "v0.0.2b 增加探险",
    "v0.0.2c Small hotfix update": "v0.0.2c 小规模热更新",
    "v0.0.2d Small update": "v0.0.2d 小规模更新",
    "Allows you to send missions to other planets": "允许您开拓其他的星球",
    "Automate some mundane task for your artisans. Gain 5% bonus to all crafting activities": "将您工匠手上的一些重复性的任务用机器自动化。所有工艺产量提升 5%",
    "Create a scalable solution for your storages. Your warehouses can now store twice as much goods": "提供可扩展的存储解决方案。您的仓库将可以存储两倍的货物",
    "Create a signal transmitter with a much stronger output. Allows you to send signals to other civilizations. Unlocks first prestige level": "创建具有更强输出的信号发射器，使您可以向其他文明发送信号。解锁第一种重置级别",
    "Further nanotechnologies investigation can further boost your civilization development": "深入研究纳米技术，可以进一步促进您的文明发展",
    "Gravity Theory": "重力理论",
    "Improve Health Diagnostics": "改良健康诊断",
    "Nano-physics": "纳米物理学",
    "New synthetic organs allow for a better treatment of your colonists. Unlocks a new medicine mode": "新的人工合成器官可以更好地治疗你的殖民者。解锁新的药物模式",
    "Organs Printing": "器官印刷",
    "Rerout energy toward terrain scanners of your expedition drones. Increases expedition loot by 30%": "将能量重新分配给探险无人机的地形扫描仪。探险战利品增加 30%",
    "Saving Expedition Drones": "节能探险无人机",
    "Using computers for diagnostics can significantly improve healthcare services. Each level increases clinic efficiency by 5%": "使用计算机进行诊断，可以显著改善医疗保健服务。每级提高诊所的医疗保健加成 5%",
    "Aesthetics Consumption": "美学消耗",
    "Capacity": "资源上限",
    "Drone masterity": "无人机精通",
    "Drone Solar Panel": "无人机太阳能电池板",
    "Each level provides 3% bonus to all your crafting productions": "每级使所有工艺产量提高 3%",
    "Electronics Mastery": "电子精通",
    "Entertainment Consumption": "娱乐消耗",
    "Every point boost all solid materials capacity 4%": "每级增加所有固体材料上限 4%",
    "Every point boost beam, brick and plate production by 4%": "每级增加木梁、砖块和金属板产量 4%",
    "Every point boost biomass, wood and nutrition production by 4%": "每级增加生物质、木材和营养物质产量 4%",
    "Every point boost electronics (wire, semiconductor, computer, etc) production 4%": "每级增加电子产品（电线、半导体、计算机等）产量 4%",
    "Every point boost fuel and rockets production by 4%": "每级增加燃料和火箭产量 4%",
    "Every point boost oxygen and water capacity 4%": "每级增加氧气和水的上限 4%",
    "Every point boost oxygen and water production 4%": "每级增加氧气和水的产量 4%",
    "Every point boost power production by 4%": "每级增加电力产量 4%",
    "Every point boost stone, ore and platinum production by 4%": "每级增加石头、矿石和铂金的产量 4%",
    "Every point decrease aesthetics consumption by 4%": "每级减少审美消耗 4%",
    "Every point decrease entertainment consumption by 4%": "每级减少娱乐消费 4%",
    "Every point decrease healthcare consumption by 4%": "每级减少医疗保健消费 4%",
    "Every point decrease religion consumption by 4%": "每级减少宗教消费 4%",
    "Every point decrease security consumption by 4%": "每级减少安全消耗 4%",
    "Every point increase drone actions output by 10%": "每级增加无人机操作的产量 10%",
    "Every point science capacity 4%": "每级增加研究点上限 4%",
    "Gas & Liquid Mastery": "液体和气体精通",
    "Healthcare Consumption": "医疗保健消耗",
    "Knowledge Point": "知识点数",
    "knowledge points (based on your current population) that can be used to purchase various upgrades": "可用于购买各种升级的知识点（获取量根据您当前的人口计算）",
    "Liquid & Gas Capacity": "液体和气体上限",
    "Materials Capacity": "固体材料上限",
    "Materials Mastery": "材料精通",
    "Mining Mastery": "采矿精通",
    "Nanobots Factory": "纳米机器人工厂",
    "Nature Mastery": "自然精通",
    "Power Mastery": "电力精通",
    "Production": "生产",
    "Religion Consumption": "宗教消耗",
    "Research Capacity": "研究上限",
    "Rocket Mastery": "火箭精通",
    "Security Consumption": "安保消耗",
    "Transmit knowledge": "传输知识",
    "Transmit Knowledge and leave the planet": "传播知识并离开这颗星球",
    "Transmit Knowledge Upgrades: 0.00 points left": "传输知识升级：还剩 0.00 点",
    "You will earn": "你将获得",
    "You will loose all your resources, upgrades, buildings and science. But you will transmit your knowledge to next colonizers, making their progress much faster.": "你将失去所有的资源、升级、建筑和科技。但是你会将你的知识传输给下一个殖民者，显著加快他们的发展进程。",
    "Your solar panel on drone grants small power generation": "升级无人机上的太阳能电池板，可提供小功率发电",
    "Allows drilling harder materials. Also improves stone output": "允许开采更硬的材料。提高石材产量",
    "Transmit knowledge only available once you have more than 50 colonizers and did \"Great Radio Transmitter\" under landing zone upgrades section": "传输知识仅在您拥有超过 50 个殖民者并升级“大型无线电发射机”后才可使用",
    "Repair Docking Module": "修复对接模块",
    "About & Change Log": "关于 & 更新日志",
    "Added 2 new science researches, 3 upgrades and 1 building": "增加了 2 个新的科学研究、3 个升级和 1 个建筑",
    "Added expedition length settings": "增加了探险时长设置",
    "Added new laws": "增加了新的法令",
    "Added notifications for new buildings/upgrades/laws": "增加了新建筑/升级/法令的通知",
    "Fixed bugs with expeditions": "修复了探险的错误",
    "Fixed some mistakes in descriptions": "修正了描述中的一些错误",
    "Re-balanced some buildings: Smelters and Monuments prices reduces, increased Smelter ore upkeep, removed Warehouse power upkeep": "重新平衡了一些建筑物：降低了冶炼厂和纪念碑的价格，增加了冶炼厂消耗的矿石，移除了仓库的电力消耗",
    "v0.0.3 Minor update": "v0.0.3 次级更新",
    "v0.0.3a Hotfix update": "v0.0.3a 热更新",
    "Aggressive cutting": "皆伐",
    "Allow limited": "允许少量使用",
    "Allow or forbid chemical processing of food": "允许或禁止对食品进行化学处理",
    "Allow or forbid forests usage for coal production": "允许或禁止森林用于煤炭生产",
    "Change forest cutting restrictions in order to balance your precious wood production and environment care": "改变森林砍伐限制，以在重要的木材生产和环境保护之间取得平衡",
    "Cistern": "蓄水池",
    "Coal burning permission": "燃煤许可",
    "Electrothermal Smelting": "电热熔炼",
    "Food chemical processing": "食品化工加工",
    "Foresters operating as normal": "林务员以正常的方式工作",
    "Forests saving": "保护森林",
    "Law Food chemical processing: Allow": "食品化学加工法：允许",
    "Learn more about impact of radiation on living organisms. Unlocks piloted missions": "详细研究辐射对生物体的影响。解锁载人飞行任务",
    "Provides additional entertainment for your people": "为您的人民提供额外的娱乐加成",
    "Radiation Study": "辐射研究",
    "Security donations": "安保捐助",
    "Time to complete:": "完成时间：",
    "Use electricity to boost metal smelting": "用电力提高金属冶炼效率",
    "Wood cutting policy": "木材切割政策",
    "Your foresters will produce 20% less wood but provide 1 point to aesthetics each": "林务员的木材产量减少 20% ，但每个林务员会提供 1 美学加成",
    "Your foresters will produce 20% more wood, but each forester providing 1.5 points penalty aesthetics.": "林务员的木材产量提升 20% ，但每个林务员会额外消耗 1.5 美学。",
    "Provides 10% bonus to nutrition production, but decrease health by 10%": "营养物质的产量提升 10% ，但降低 10% 的医疗保健",
    "Science buildings consume 25% more electricity but provide 5% more science capacity": "地面科学建筑额外消耗 25% 的电力，但提供额外 10% 的研究点上限",
    "Science buildings consume 75% more electricity but provide 10% more science capacity": "地面科学建筑额外消耗 75% 的电力，但提供额外 10% 的研究点上限",
    "Smelters operate as usually": "冶炼厂以正常的方式运作",
    "Smelters start using 0.3 power each, but produce 30% more metal": "每级冶炼厂会消耗 0.3 的电力，但金属产量提升 30%",
    "Thermal plants are 10% more efficient but now using 0.01 wood each": "热电厂的电力产量提高 10%，但每座热电厂每秒需消耗 0.01 木材",
    "Thermal plants are 25% more efficient but now using 0.03 wood each": "热电厂的电力产量提高 25%，但每座热电厂每秒需消耗 0.03 木材",
    "Thermal plants dont using wood": "热电厂不消耗木材",
    "Waterpumps consume twice as much power. Increase healthcare by 3 per Waterpump.": "水泵消耗两倍的电力，每座水泵增加 3 医疗保健。",
    "3D Printing": "3D 打印",
    "Advanced Biology": "高级生物学",
    "Cargo Cranes": "货运起重机",
    "Further advance biology knowledge": "进一步提高生物学知识",
    "Happiness affects your colonists productivity. Has no effect on machinery production.": "幸福会影响您的殖民者的生产力。对机械生产无影响。",
    "Learn more about surviving and creating in high gravity conditions": "进一步研究如何在高重力条件下生存和创造",
    "Repair Research Module": "修复研究模块",
    "Unlock new powerful tools to boost your civilization development": "解锁全新的强大工具，来促进你的文明发展",
    "Create insulation for combustion cameras. Improves thermal plant efficiency by 20%": "为燃烧室增加绝缘层。将热电厂的电力产量提高 20%",
    "Create new electrolyte solution to improve power capacity": "创造新的电解质溶液以提高电力上限",
    "Create platinum tools to provide additional production boost. Increases wood production and wooden beam, brick and metal plate crafting": "创造铂金工具，以提供额外的产量提升。提升木材、木梁、砖块和金属板的产量",
    "Heat Insulating Cameras": "隔热舱",
    "Improves knowledge about conductivity of various materials": "增进对各种材料导电性的了解",
    "Learn about new use of your biomass": "了解您的生物质的全新用途",
    "Organic Chemistry": "有机化学",
    "Unlocks new aspects of your civilization development": "解锁您文明发展的全新角度",
    "Using composite metals allows to increase crafting output for metal plates and wires": "使用复合金属可以增加金属板和电线的产量",
    "3D Printers Construction": "3D 打印建造",
    "Create prefabricated sections on your 3D printers. Further decreases building cost": "用 3D 打印机制造建筑的预制部分。进一步降低建筑升级成本的提升率",
    "Bacteria Selection": "菌种选择",
    "Improved Bio Technology": "改良生物技术",
    "Increases electricity consumption by incubators, but incubators start producing oxygen": "提升细菌培养箱的耗电量，但细菌培养箱可以生产氧气",
    "Increases stone, ore and biomass storage from storage sections and hubs by 25%": "存储区域和存储中心的石头、矿石和生物质上限增加 25%",
    "Hide resources": "隐藏资源",
    "Added mobile support (Experimental and might be buggy)": "添加了移动端支持（实验性，可能存在问题）",
    "Items that require more capacity are now highlighted differently": "需要更多资源上限的项目现在以不同的方式突出显示",
    "v0.0.3b Mobile friendly UI version": "v0.0.3b 移动端友好界面版本",
    "\"Foresters produce a normal amount of wood": "\“林务员生产正常数量的木材",
    "A zone with rich greenery provides a way to relax for your people": "一片绿意盎然的区域，为您的人民提供放松的方式",
    "Add few new researches and buildings": "增加了一些新的研究和建筑物",
    "Added +/-5 controls for jobs and crafters": "为工作和工匠添加了 +/-5 控制按钮",
    "Added highlight for buildings that use howered resource": "鼠标移动到资源上时，需要使用它的项目将会高亮",
    "Administrative Service Center": "行政服务中心",
    "Advanced Metallurgy": "高级冶金技术",
    "Advanced Water Filtering": "高级水过滤技术",
    "Aggressive Cutting": "皆伐",
    "All food is grown organically and served unprocessed": "所有食物都是有机种植的，无需加工即可直接食用",
    "Allow All": "全部允许",
    "Allow or forbid wood usage as coal": "允许或禁止将木材用作煤炭",
    "Allow using a minor amount of wood for thermal plants. Thermal plants are 10% more efficient, but consume 0.01 wood each": "允许热电厂使用少量木材。热电厂的电力产量提高 10%，但每座热电厂每秒需消耗 0.01 木材",
    "Automate some mundane tasks of your artisans. Gain 5% bonus to all crafting": "将您工匠手上的一些重复性的任务用机器自动化。所有工艺产量提升 5%",
    "Automatically collects ore": "自动收集矿石",
    "Automatically collects stone": "自动收集石头",
    "Automatically produces biomass": "自动生产生物质",
    "Better understanding of quantum physics can significantly boost the productivity of your civilization": "更好地理解量子物理学，显著提高你的文明的生产力",
    "Bio Laboratory": "生物实验室",
    "Building Materials Factory": "建筑材料工厂",
    "Bureaucratic center of your colony. Each level decrease public services requirements of your people by 2%": "你殖民地的官僚中心。 每个级别都会将您人民的公共服务需求降低 2%",
    "Burns biomass to generate power": "燃烧生物质发电",
    "Burns wood to smelt ore into metal": "燃烧木材将矿石熔炼成金属",
    "Change forest cutting restrictions in order to balance your wood production and environmental effects": "改变森林砍伐限制以平衡木材生产和环境影响",
    "Changes the water filtration method": "改变水过滤方法",
    "Chemical Analysis": "化学分析",
    "Clinics are working in a regular way, providing service using only reliable and tested methods": "诊所正常运作，只使用可靠和经过测试的方法提供服务",
    "Coal Burning Regulations": "燃煤法规",
    "Colonists now assigned automatically to artisans once you run out of free crafters but have free colonists": "一旦您用完免费工匠但有免费殖民者，殖民者现在会自动分配给工匠",
    "Composite Metals": "复合金属",
    "Compresses oxygen for easier storage": "压缩氧气以便于储存",
    "Computer": "电脑",
    "Create a scalable solution for your storage. Your warehouses can now store twice as much goods": "为您的存储创建可扩展的解决方案。 您的仓库现在可以存储两倍的货物",
    "Create prefabricated sections on your 3D printers. Further decreases building cost by 10%": "在 3D 打印机上创建预制部分。 将建筑成本进一步降低 10%",
    "Dedicated center of exploration provides a 5% multiplicative bonus to your expeditions outcome. Requires power and oxygen": "专用探索中心为您的探险结果提供 5% 的乘数奖励。 需要电力和氧气",
    "Dig the soil for organic matter to gather some biomass": "挖掘有机质土壤以收集一些生物质",
    "Drill Ore": "钻矿石",
    "Drill Rock": "钻岩",
    "Efficient Processing": "高效加工",
    "Electronics Factory": "电子厂",
    "Electrothermal": "电热",
    "Encourage limited cutting and frequent replanting. Foresters produce 20% less wood, but each forester increases aesthetics by 1": "鼓励有限的砍伐和频繁的重新种植。 林务员减少 20% 的木材产量，但每个林务员增加 1 美感",
    "Encourage production at the cost of pretty landscape. Foresters produce 20% more wood, but each forester reduces aesthetics by 1.5": "以美丽的风景为代价鼓励生产。 林务员多生产 20% 的木材，但每个林务员减少 1.5 美感",
    "Expedition button now on the top": "探险按钮现在位于顶部",
    "Extra Rations": "额外口粮",
    "Fix bug with building efficiency calculation": "修复建筑效率计算的错误",
    "Fixed descriptions": "修复描述",
    "Fixed display on album-oriented screens": "修复了在面向专辑的屏幕上的显示",
    "Food Chemical Processing": "食品化学加工",
    "Forbid All": "禁止所有",
    "Forest plants operate normally": "林业植物正常运转",
    "Forest plants produce 50% less oxygen, but provide a small bonus to aesthetics": "森林植物产生的氧气减少 50%，但对美观有一点好处",
    "Forest plants produce 50% less oxygen, but provide a small bonus to entertainment": "森林植物产生的氧气减少了 50%，但为娱乐提供了一点好处",
    "Forest Preservation": "森林保护",
    "Forest Use": "森林利用",
    "Genetic modification of organisms can help boost nutrition production by 5% and biomass production by 10%": "生物体的基因改造可以帮助提高 5% 的营养产量和 10% 的生物量产量",
    "Grant your scientists better tools to work with. Science buildings consume 25% more power, but provide 5% more research capacity": "为您的科学家提供更好的工作工具。 科学建筑多消耗 25% 的电力，但提供多 5% 的研究能力",
    "Grant your scientists state-of-the-art tools to work on prototypes. Science buildings consume 75% more power, but provide 10% more research capacity": "为您的科学家提供最先进的工具来处理原型。 科学建筑多消耗 75% 的电力，但提供多 10% 的研究能力",
    "Guards Rations": "卫兵口粮",
    "Improved Electrolytes": "改进的电解质",
    "Improved smelting technique increases smelter output by 20%": "改进的冶炼技术使冶炼厂产量提高 20%",
    "Incentivize your police force to work better with extra rations. Policemen consume extra 0.5 nutrition and 0.2 water each, but provide a better service": "激励你的警察部队用额外的口粮更好地工作。 警察每人多消耗 0.5 营养和 0.2 水，但提供更好的服务",
    "Limited Use": "有限使用",
    "Literally. There is too little space on our planet, isn't it?": "字面上地。 我们星球上的空间太小了，不是吗？",
    "Lungs of the Colony": "殖民地之肺",
    "Major Support": "主要支持",
    "Minor Regulations": "细则",
    "Minor Support": "次要支持",
    "Multi-Stage": "多级",
    "Nature Reserves": "自然保护区",
    "New synthetic organs allow for a better treatment of your colonists. Unlocks a new social law": "新的合成器官可以更好地治疗你的殖民者。 解锁新的社会法则",
    "No Extra Support": "无额外支持",
    "Oxygen Compressor": "氧气压缩机",
    "Pandemic Research Center": "流行病研究中心",
    "Park Zone": "公园区",
    "Plastic": "塑料",
    "Platinum Drills": "铂金钻头",
    "Platinum Tools": "白金工具",
    "Process food to increase shelf life and nutritional value at the cost of its quality. Provides 10% bonus to nutrition production, but decrease healthcare by 10%": "加工食品以牺牲其质量为代价来延长保质期和营养价值。 为营养生产提供 10% 的奖励，但减少 10% 的医疗保健",
    "Provides 25 research storage and boosts science output by 2% multiplicatively": "提供 25 项研究存储并以乘法方式提高 2% 的科学产出",
    "Provides additional research capacity and generation": "提供额外的研究能力和生成",
    "Provides even more research storage and boost research output by 5% multiplicatively": "提供更多的研究存储空间，将研究产出提高 5%",
    "Pumps water from underground to provide a steady supply": "从地下抽水以提供稳定的供应",
    "Quick": "快的",
    "Recover your power": "恢复你的力量",
    "Regular": "常规的",
    "Regulate rations for your policemen to motivate them to work harder": "规范警察的口粮，以激励他们更加努力地工作",
    "Regulate the chemical processing of food": "规范食品的化学加工",
    "Regulates the use of experimental drugs": "规范实验药物的使用",
    "Research Focus": "研究重点",
    "Researches viruses and other diseases. Each level decrease colony healthcare requirements by 5%": "研究病毒和其他疾病。 每个级别将殖民地医疗保健要求降低 5%",
    "Residential Module": "住宅模块",
    "Rich forest area that generates oxygen": "富饶的产氧林区",
    "Rocket Science": "火箭科学",
    "Science buildings operate normally": "科学楼正常运行",
    "Semiconductor": "半导体",
    "Setting research as priority through providing better equipment to your scientists can boost the development of your colony, but it won't be cheap": "通过为你的科学家提供更好的设备将研究放在首位可以促进你的殖民地的发展，但它不会便宜",
    "Show researched": "显示已研究",
    "Smelters use 0.3 power each, but produce 30% more metal": "每个冶炼厂消耗 0.3 功率，但多生产 30% 的金属",
    "Smelting Method": "冶炼法",
    "Social Research Center": "社会研究中心",
    "Spend power to drill some rocks for stone": "花力钻石取石",
    "Spend power to mine some ore": "花费力量开采一些矿石",
    "Storage Container": "储存容器",
    "Storage Hub": "存储中心",
    "Storage Section": "存储部分",
    "There is enough smoke as is, the wood is better used elsewhere. Thermal plants dont use wood": "有足够的烟雾，木头最好用在别处。 热电厂不使用木材",
    "Time to inflict another industrial revoluion. Thermal plants are 25% more efficient, but consume 0.03 wood each": "是时候发动另一场工业革命了。 热电厂的效率提高 25%，但每台消耗 0.03 木材",
    "Traditional": "传统的",
    "Unregulated Use": "不受管制的使用",
    "Upgrade Wiring": "升级接线",
    "Use alternative methods to boost metal smelting": "使用替代方法促进金属冶炼",
    "Using computers for diagnostics can significantly improve healthcare services. Each level increases healthcare from clinics by 5%": "使用计算机进行诊断可以显著改善医疗保健服务。 每个级别增加 5% 的诊所医疗保健",
    "Using experimental medicine reduces healthcare from clinics, but they also provide a boost to max research": "使用实验医学减少了诊所的医疗保健，但它们也促进了最大的研究",
    "v0.0.4 Minor update": "v0.0.4 次要更新",
    "v0.0.4a Hotfix": "v0.0.4a 修补程序",
    "Warehouses now provide nutrition capacity": "仓库现在提供营养能力",
    "Water Pump": "水泵",
    "Water pumps are working in regular way": "水泵正常工作",
    "Water pumps consume twice as much power, but increase healthcare by 3 each": "水泵消耗两倍的功率，但每个增加 3 个医疗保健",
    "Water Storage": "蓄水",
    "Woodcutting Policy": "伐木政策",
    "Your forest plants can be used in different ways to provide various advantages": "您的森林植物可以以不同的方式使用，以提供各种优势",
    "Add a solar panel to your starting drone that grants it small power generation": "为您的起始无人机添加一块太阳能电池板，使其产生少量电力",
    "Computers kickstarted a second industrial revolution and are very important in automating the industry. Every point boosts electronics (wire, semiconductor, computer, etc) production by 4%": "计算机启动了第二次工业革命，对工业自动化非常重要。 每一点都会使电子产品（电线、半导体、计算机等）的产量提高 4%",
    "Drone Mastery": "无人机精通",
    "Each space colony starts with a rocket trip, and you are good at making rockets. Every point boosts fuel and rockets production by 4%": "每个太空殖民地都从一次火箭之旅开始，而你擅长制造火箭。 每一点都会使燃料和火箭产量提高 4%",
    "Extensive knowledge of biology and farming helps with spreading greenery on inhospitable worlds. Every point boosts biomass, wood and nutrition production by 4%": "广泛的生物学和农业知识有助于在荒凉的世界上传播绿色植物。 每一点都会将生物量、木材和营养生产提高 4%",
    "Gas & Liquid Capacity": "气体和液体容量",
    "In every prayer, a strength of the spirit. From strong spirit, a stronger body. Every point decreases religion requirements by 4%": "在每一个祈祷中，都有精神的力量。 来自强大的精神，更强大的身体。 每一点减少 4% 的宗教要求",
    "Know when to de-escalate, know when to intervene. Every point decreases security requirements by 4%": "知道何时降级，知道何时进行干预。 每一点降低 4% 的安全要求",
    "Knowledge: Drone Solar Panel": "知识：无人机太阳能电池板",
    "Learn how to use your drone with more efficiency. Every point increases the output from drone actions by 10%": "了解如何更高效地使用无人机。 每一点增加无人机行动的输出 10%",
    "More industry facilities require more power. Learn how to build better batteries to increase your power storage. Every point boosts power capacity by 4%": "更多的工业设施需要更多的电力。 了解如何制造更好的电池以增加您的电力储存。 每一点增加4%的能量容量",
    "Power Capacity": "电源容量",
    "Pretty environment is surprisingly effective at boosting productivity. Every point decreases aesthetics requirements by 4%": "漂亮的环境在提高生产力方面出奇地有效。 每点降低4%的审美要求",
    "Pursuit of knowledge never stops for someone as cursious as you. Every point increases research capacity by 4%": "对于像您这样充满好奇心的人来说，对知识的追求永远不会停止。 每一点增加4%的研究能力",
    "Refined materials lie at the core of all complex structures. Every point boosts beam, brick and plate production by 4%": "精制材料是所有复杂结构的核心。 每一点都会使梁、砖和金属板的产量提高 4%",
    "Refining Mastery": "精炼精通",
    "You know all about stocking containers to the top. Every point boosts all solid materials capacity by 4%": "你知道所有关于储存容器到顶部的知识。 每一点提升所有固体材料容量4%",
    "You know how important full oxygen tank and filled water storage is. Every point boosts oxygen and water capacity by 4%": "您知道充满氧气罐和充满水的存储有多重要。 每一点增加4%的氧气和水容量",
    "Your experience with electrical grids helps with power optimization. Every point boosts power production by 4%": "您在电网方面的经验有助于优化电力。 每一点增加4%的发电量",
    "Repair mining machinery. Improves your automatic quarries and mines by 50%": "维修矿山机械。 将您的自动采石场和矿山提高 50%",
    "Repair Storage Module": "修复存储模块",
    "Repair the storage module. Allows building storage sections": "修复存储模块。 允许建造存储部分",
    "Improved Combustion": "改善燃烧",
    "Improved Mine": "改进的矿山",
    "Improved Quarry": "改良采石场",
    "Improves power storage by +20": "将能量储存提高 +20",
    "Modify conveyor belts of your mines to increases their output by 25%": "修改矿山的传送带，使其产量提高 25%",
    "Modify conveyor belts of your quarries to increases their output by 25%": "修改采石场的传送带，使其产量提高 25%",
    "Regulate the flow of raw materials to stabilize the combustion process. Increases thermal plant output by 25%": "调节原料流量以稳定燃烧过程。 将热电厂输出增加 25%",
    "Modify the delivery mechanism to work at higher speeds. Increases thermal plant output by 25%": "修改交付机制以更高的速度工作。 将热电厂输出增加 25%",
    "Improved Thermal Plants": "改进的热电厂",
    "Create a better environment for biological growth. Increases incubators output by 25%": "为生物生长创造更好的环境。 将孵化器产量提高 25%",
    "Allows building extra batteries to store power. Also, improves power output by 20%": "允许建造额外的电池来储存电力。 此外，提高 20% 的功率输出",
    "Add oxygen to maintain a stronger pressure. Thermal plants are 50% more efficient, but now require oxygen": "添加氧气以保持更强的压力。 热电厂的效率提高了 50%，但现在需要氧气",
    "Allows building oxygen compressors to store more oxygen": "允许建造氧气压缩机以储存更多氧气",
    "Oxidize Combustion Cameras": "氧化燃烧相机",
    "Oxygen Compressors": "氧气压缩机",
    "Repair the supply module to start generating a trace amount of nutrition": "修复补给模块开始产生微量营养",
    "Repair the research module to allow colonists to work as researchers and produce research": "修复研究模块，让殖民者可以作为研究人员工作并进行研究",
    "Repairs sorting machinery. Gives 75% bonus to stone, ore and biomass storage": "修理分拣机。 为石头、矿石和生物质储存提供 75% 的加成",
    "Exo Farming": "外来农业",
    "Learn how to work with soil on a foreign planet. Allows employing farmers": "了解如何在外星球上处理土壤。 允许雇用农民",
    "Aid Buildings": "援助建筑",
    "Artificial Organs": "人工器官",
    "Conductive Nano-Particles": "导电纳米粒子",
    "Crafts complex goods out of raw materials or other goods": "用原材料或其他物品制作复杂的物品",
    "High Energy Physics": "高能物理",
    "Huge storage building.": "巨大的仓库建筑。",
    "Impulse Engine": "脉冲发动机",
    "Lay new wires to further increase power supply by 5%": "铺设新电线，进一步增加供电5%",
    "Learn more about ionizing radiation impact on chemical reactions. Unlocks new upgrades": "详细了解电离辐射对化学反应的影响。 解锁新的升级",
    "Maintains forests, plants and cuts wood": "维护森林、植物和砍伐木材",
    "Patrols the colony and provides security": "巡逻殖民地并提供安全保障",
    "Photochemistry": "光化学",
    "Place where people can enjoy watching animals similar to ones they saw before on Earth": "人们可以在这里欣赏与他们以前在地球上见过的动物相似的动物",
    "Pluto Monument": "冥王星纪念碑",
    "Provides research capacity bonus": "提供研究能力加成",
    "Replace traditional fuel burning with modern approach, based on ionized plasma stream accelerated by powerful magnetic field. Further reduces rocket consumption by 20% for all space structures and ships": "基于强大磁场加速的电离等离子流，用现代方法取代传统燃料燃烧。 所有空间结构和船舶的火箭消耗进一步减少 20%",
    "Satellite Solar Panel": "卫星太阳能电池板",
    "Smelting Complex": "冶炼综合体",
    "Space Storage Hub": "空间存储中心",
    "Space Telescope": "太空望远镜",
    "This hostile and hot planet, literally made of iron will remind your people of the god of the Underground. Requires a lot resources, but provides significant motivation to religious people": "这个充满敌意和炎热的星球，字面上是由铁制成的，会让你的人民想起地下之神。 需要大量资源，但为宗教人士提供了重要的动力",
    "Tills the soil and plants crops to generate nutrition": "耕种土壤和种植作物以产生营养",
    "Uses ore and power to produce big amount of metal. Produces a lot pollution, so placing it on uninhabited place is preferred": "使用矿石和电力生产大量金属。 产生大量污染，最好放置在无人居住的地方",
    "Using 3D printers in creating organs to heal people can significantly boost healthcare, but religious people won't like it": "使用 3D 打印机制造器官来治愈人类可以显著促进医疗保健，但宗教人士不会喜欢它",
    "Using new conductive particles improves the capacity of your batteries and accumulators by 20%": "使用新的导电颗粒可将电池和蓄电池的容量提高 20%",
    "We are so miserable comparing to bright far stars and supernova explosions. Learn how to deal with more energy to expand your capabilities even more. Unlocks further flights and more important researches": "与明亮的遥远恒星和超新星爆炸相比，我们是如此悲惨。 了解如何处理更多能量以进一步扩展您的能力。 解锁更多航班和更重要的研究",
    "Works at the lab to generate research": "在实验室工作以进行研究",
    "Works at the mine to gather ore": "在矿山工作以收集矿石",
    "Works at the quarry to mine stone": "在采石场工作以开采石头",
    "Zoo": "动物园",
    "Dig an underground storage for your basic materials, which increases storage facilities by 50%": "为你的基本材料挖一个地下仓库，增加 50% 的存储设施",
    "Exo Biology": "外来生物学",
    "Expand your mining shaft network. Improves stone and ore output by 20%": "扩展您的矿井网络。 提高 20% 的石头和矿石产量",
    "Hide researched": "隐藏研究",
    "High-Gravity Engineering": "高重力工程",
    "Learn about engineering specific to high gravity conditions": "了解特定于高重力条件的工程",
    "Learn how to work with exotic ores. Unlocks crafting and artisan job": "了解如何处理外来矿石。 解锁手工艺和工匠工作",
    "Learn more about the atmosphere and its weather patterns. Unlocks new upgrades and buildings": "了解更多关于大气及其天气模式的信息。 解锁新的升级和建筑",
    "Provide some basic tools to your farmers. Improves farming output by 10%": "为您的农民提供一些基本工具。 农业产出提高 10%",
    "Survey deeper layers to find underground water deposits. Improves water production by 20%": "调查更深层以寻找地下水矿藏。 将产水量提高 20%",
    "Unlock new ways of providing services to your growing colony": "解锁为您不断壮大的殖民地提供服务的新方式",
    "Advanced Anatomy": "高级解剖学",
    "Advanced flora researches unlocks new tools to boost agriculture": "先进的植物群研究解锁了促进农业发展的新工具",
    "Advanced Genetics": "高级遗传学",
    "Allows to further advance your understanding of celestial bodies through new scientific telescopes": "允许通过新的科学望远镜进一步提高您对天体的理解",
    "Better understanding of gravitational forces on flying objects will help with rocket launches. Allows you to send missions to other planets": "更好地了解飞行物体的重力将有助于火箭发射。 允许您向其他星球发送任务",
    "Civil Service": "公务员",
    "Electronics": "电子产品",
    "Exo Flora": "外来植物群",
    "Exo Forestry": "外来林业",
    "Further advance biology knowledge. Create new methods of boosting nutritional value and unlock new research": "进一步提高生物学知识。 创造提高营养价值的新方法并开启新的研究",
    "Further advances crafting and metal processing": "进一步推进手工艺和金属加工",
    "Further research into nanotechnology can increase your civilization productivity even more": "对纳米技术的进一步研究可以进一步提高您的文明生产力",
    "Further research precious minerals available at your new habitat": "进一步研究新栖息地的珍贵矿物质",
    "Learn about new uses for biomass": "了解生物质的新用途",
    "Learn how to grow something bigger in this empty terrain": "了解如何在这片空旷的土地上种植更大的东西",
    "Learn how to produce electronics and incorporate it": "了解如何生产电子产品并将其整合",
    "Learn more about spacetime continuum. Unlocks new tech and further traveling": "了解有关时空连续体的更多信息。 解锁新技术和更远的旅行",
    "Material Knowledge": "材料知识",
    "Nano-Physics": "纳米物理学",
    "Push further your genetics knowledge to learn how species can be modified": "进一步推动您的遗传学知识，了解如何改造物种",
    "Relativity Physics": "相对论物理学",
    "Study the material interactions of elements in this foreign environment. Unlocks new materials and upgrades": "研究元素在这种外国环境中的物质相互作用。 解锁新材料和升级",
    "Unlock new civil service jobs and buildings": "解锁新的公务员职位和建筑",
    "Unlock new powerful tools that will help you boost your colony development": "解锁新的强大工具，帮助您促进殖民地发展",
    "Unlocks new aspects of your colony development": "解锁您的殖民地发展的新方面",
    "Cultivate a special flammable biomass that produces more heat when burnt, increasing power generation from your thermal plants by 50%": "培育一种特殊的易燃生物质，燃烧时会产生更多热量，使热电厂的发电量增加 50%",
    "Flammable Biomass": "易燃生物质",
    "Plant some special flora in your incubators that increases bacterial growth when decomposing. Makes incubators 50% more productive": "在您的培养箱中种植一些特殊的植物群，这些植物群会在分解时增加细菌的生长。 使孵化器的生产力提高 50%",
    "Start adding fertilizers to the soil. Your farmers will start using 0.1 biomass each, but it will increase their nutrition output by 30%": "开始向土壤中添加肥料。 您的农民每人将开始使用 0.1 生物质，但这将使他们的营养产出增加 30%",
    "Broad-Leaved Forests": "阔叶林",
    "Create new tools for your foresters to increases wood output by 25%": "为林务员打造新工具，将木材产量提高 25%",
    "Knowledge: Research Capacity": "知识：研究能力",
    "Start planting trees with broad leaves. Forests plants generate oxygen": "开始种植叶子宽阔的树木。 森林植物产生氧气",
    "Using wooden furniture allows housing an extra colonist in every cabin": "使用木制家具可以在每个小屋中容纳一名额外的殖民者",
    "Wooden Furniture": "木制家具",
    "Create metal tools to improve wood output of foresters by 50%": "制造金属工具，将林务员的木材产量提高 50%",
    "Replace old parts with metal, which improves stone and ore output by 50%": "用金属替换旧部件，提高 50% 的石头和矿石产量",
    "Use local-sourced metal to improve storage containers. Improves all resources storages by 50%": "使用本地采购的金属来改进存储容器。 将所有资源存储提高 50%",
    "Use more conductive materials in your wiring. Improves power capacity by 75% and output by 25%": "在布线中使用更多导电材料。 功率容量提升75%，输出功率提升25%",
    "Forest ecosystem is favourable for plant growth. Forest plants now boost biomass production by 2% each": "森林生态系统有利于植物生长。 森林植物现在将每个植物的生物质产量提高 2%",
    "Fruit Trees": "果树",
    "Plant fruit trees in your forests. Forest plants now also provide nutrition": "在你的森林里种植果树。 森林植物现在也提供营养",
    "Teach your foresters better conservation methods. Improves foresters output by 25%": "教你的林务员更好的保护方法。 将林务员的产出提高 25%",
    "Allows colonist miners to extract platinum": "允许殖民主义矿工提取铂",
    "Better filtration methods allow reusing more water. Improves water output by 25%": "更好的过滤方法可以重复使用更多的水。将水量提高25%",
    "Create new electrolyte solution to improve power capacity by 25%": "创造新的电解质溶液，将电力容量提高25%",
    "Platinum Mining": "铂矿",
    "Study chemical reactions. Improves science output by 25%": "研究化学反应。将科学产出提高25%",
    "Create plans for a new housing unit": "制定新住房单元的计划",
    "Society research allows better understanding of human needs. Unlocks new ways to boost population happiness": "社会研究可以更好地了解人类的需求。解锁提高人口幸福感的新方法",
    "Unlocks new scientific building for studying sociology": "解锁学习社会学的新科学建筑",
    "Better Photo-Elements": "更好的照片元素",
    "Improved photo-element technology allows you to boost solar satellites and solar panels output by 20%": "改进的光电元件技术可让您将太阳能卫星和太阳能电池板的输出提高 20%",
    "New methods of lithography improve assembling processes. Boosts computers production by 10%": "新的光刻方法改进了组装工艺。 将计算机产量提高 10%",
    "Using lasers to cut out metal parts significantly simplifies plates and wires production. Increased plate and wire production by 10%": "使用激光切割金属零件可以显著简化金属板和线材的生产。 金属板和线材产量增加 10%",
    "Create platinum tools to provide additional production boost. Increases wood production and beam, brick and plate crafting by 10%": "创建白金工具以提供额外的产量提升。 木材产量以及梁、砖和金属板制作增加 10%",
    "Replace drill heads with platinum. Improves stone and ore output by 40%": "用铂金替换钻头。 石头和矿石产量提高 40%",
    "Install surveillance cameras around your colony. Improves policemen efficiency by 25%": "在你的殖民地周围安装监控摄像头。 提高警察效率 25%",
    "Rerout extra energy toward terrain scanners of your expedition drones. Increases expedition loot by 30%": "将额外的能量重新分配给探险无人机的地形扫描仪。 探险战利品增加 30%",
    "Added new late game content": "添加了新的游戏后期内容",
    "Added second prestige level": "增加了第二个声望等级",
    "Decreased some upgrades bonuses": "减少了一些升级奖励",
    "Fixed some prestige upgrades descriptions": "修复了一些声望升级描述",
    "Fixed weird scroll behaviour on mobile": "修复了移动设备上奇怪的滚动行为",
    "Increased factories bonuses": "增加工厂奖励",
    "v0.0.5 Content Update": "v0.0.5 内容更新",
    "You start with broken ship on hostile planet. Your main goal is to prepare for first colonists to arrive, and build prosperous civilisation": "你从敌对星球上的破损船只开始。 你的主要目标是为第一批殖民者的到来做好准备，并建立繁荣的文明",
    "Gravitational Lensing Study": "引力透镜研究",
    "Improved smelting technique increases smelter output by 15%": "改进冶炼技术使冶炼厂产量提高 15%",
    "In every prayer, a strength of the spirit. From strong spirit, a stronger body. Every point decreases religion requirements by 4": "每一次祈祷都充满精神的力量。 源自强大的精神，更强大的身体。 每一点都会使宗教要求降低 4 点",
    "Know when to de-escalate, know when to intervene. Every point decreases security requirements by 4": "知道何时降级，知道何时进行干预。 每一点都会使安全要求降低 4",
    "Knowledge: Electronics Mastery": "知识：电子精通",
    "Learn more about the powerful physical effect from massive celestial bodies that causes the curvature of spacetime. Provides more knowledge about the universe and surrounding stars and boosts space telescope effect by 40%": "了解更多关于大质量天体导致时空弯曲的强大物理效应。 提供更多关于宇宙及周围恒星的知识，太空望远镜效果提升40%",
    "Photo-Lithography": "光刻法",
    "Platinum Mine": "铂矿",
    "Pretty environment is surprisingly effective at boosting productivity. Every point decreases aesthetics requirements by 4": "漂亮的环境对于提高生产力有着惊人的效果。 每一点，美观度要求降低4",
    "Provides stable platinum source, but requires a lot power to operate": "提供稳定的铂源，但需要大量电力才能运行",
    "Radiative Catalyzation": "辐射催化",
    "Radiative Dissociation": "辐射解离",
    "Use radiation in chemical reactions to boost your fuel output by 20%": "在化学反应中使用辐射可将燃料产量提高 20%",
    "Use your advances in quantum physics and material knowledge to boost production of tiny things. Unlocks new upgrades": "利用您在量子物理学和材料知识方面的进步来促进微小物体的生产。 解锁新升级",
    "Using radiation can improve dissociation process, improving your electrolytes efficiency.  Improves your power storage facilities by 20%.": "使用辐射可以改善解离过程，提高电解质效率。 将您的电力存储设施改善 20%。",
    "You are not easily bored and know how to distract people from boredom. Every point decreases entertainment requirements by 4": "你不容易感到无聊，并且知道如何分散人们的无聊感。 每一点都会减少 4 点娱乐要求",
    "Unlocks nanobots factory building on your satellite that provides bonus to all crafting": "解锁卫星上的纳米机器人工厂建筑，为所有手工艺提供奖励",
    "Fixed bug in formula for second layer prestige bonus calculation": "修复了第二层声望奖励计算公式中的错误",
    "Fixed bug when Organics mastery was missing": "修复了有机物精通缺失时的错误",
    "Fixed descriptions for some prestige upgrades": "修复了一些声望升级的描述",
    "Fixed layout issues on some devices": "修复了某些设备上的布局问题",
    "v0.0.5a Small Hotfix Update": "v0.0.5a 小修补程序更新",
    "Knowledge: Gas & Liquid Capacity": "知识：气体和液体容量",
    "Knowledge: Materials Capacity": "知识：材料能力",
    "Knowledge: Mining Mastery": "知识：采矿精通",
    "Knowledge: Power Capacity": "知识：电源容量",
    "Knowledge: Power Mastery": "知识：力量精通",
    "Learn how to use your drone with more efficiency. Every point increases the output from drone actions by 20%": "了解如何更高效地使用无人机。 每一点都会使无人机行动的输出增加 20%",
    "Organics Mastery": "有机物精通",
    "Knowledge: Refining Mastery": "知识：精炼精通",
    "Learn how to artificially change animals through gene modification. Unlocks Zoo": "了解如何通过基因改造人为地改变动物。 解锁动物园",
    "Animal Gene Modification": "动物基因改造",
    "Plasma Knife": "等离子刀",
    "Transistor Photo-Printing": "晶体管照片打印",
    "Use hot ionized gas to cut metal. Improves plates, wires and rockets output by 10%": "使用热电离气体切割金属。 将金属板、线材和火箭产量提高 10%",
    "Use photolithography to boost your computers production by 20%": "使用光刻技术将计算机产量提高 20%",
    "Advanced Molecular Physics": "高级分子物理学",
    "Advanced Polymers": "先进聚合物",
    "Flora Plant": "植物区系",
    "Hot and wet climate allows you to produce more biomass here. Requires power however to operate": "炎热潮湿的气候使您可以在这里生产更多的生物质。 但需要电源才能运行",
    "Learn how to generate more sophisticated compounds to boost your development": "了解如何生成更复杂的化合物来促进您的发展",
    "Learn more about high temperature physics to unlock new source of power": "了解有关高温物理学的更多信息，以解锁新的能源",
    "Organics Processing Factory": "有机物加工厂",
    "Plant large forest plantations to boost your foresters. Each level provides 3% bonus to wood output, but consumes biomass and water": "种植大型人工林来增强您的林务人员的能力。 每个级别为木材产量提供 3% 的奖励，但会消耗生物质和水",
    "Thermonuclear synthesis": "热核合成",
    "Tropic Forest Plant": "热带森林植物",
    "Uses suitable climate to decrease cost of organic materials synthesis. Each level increase rubber and plastic output by 5%": "利用适宜的气候来降低有机材料合成的成本。 每级增加橡胶和塑料产量5%",
    "Utilize a highly ionized environment to generate sophisticated, radiation- and temperature-hardened polymers. Unlocking new upgrades": "利用高度电离的环境来生成复杂的、辐射和温度硬化的聚合物。 解锁新升级",
    "Venus Buildings": "金星建筑物",
    "Fixed some other small bugs": "修复了其他一些小错误",
    "Fixed UI bug on mobile on pages with small amount of content": "修复了移动设备上内容较少的页面上的 UI 错误",
    "Hold Ctrl to decrease to 0": "按住Ctrl键减少到0",
    "Hold Ctrl to increase to": "按住 Ctrl 键可增加至",
    "Hold Shift to decrease by 5": "按住 Shift 键减少 5",
    "Hold Shift to increase by 5": "按住 Shift 键可增加 5",
    "Increased some buildings efficiency": "提高了一些建筑物的效率",
    "v0.0.5b Small Hotfix Update": "v0.0.5b 小修补程序更新",
    "You can now press Shift or Ctrl to batch enable/disable buildings": "您现在可以按 Shift 或 Ctrl 批量启用/禁用建筑物",
    "Allows producing electrical power by burning biomass": "允许通过燃烧生物质发电",
    "Improves power output": "提高电力输出",
    "Knowledge: Nature Mastery": "知识：自然精通",
    "Repair Machinery": "维修机械",
    "Continued goals": "持续目标",
    "Fixed some bugs in buildings bonuses": "修复了建筑物奖励中的一些错误",
    "Fixed some bugs in prestige mechanics": "修复了声望机制中的一些错误",
    "Knowledge of Ancestors": "祖先的知识",
    "Knowledge Upgrade: Research Capacity": "知识升级：研究能力",
    "v0.0.5c Small Hotfix Update": "v0.0.5c 小热修复更新",
    "Create insulation for combustion chambers. Improves thermal plant efficiency by 20%": "为燃烧室创造隔热层。 将热电厂效率提高 20%",
    "Heat Insulating Chambers": "隔热室",
    "Using composite metals allows to increase crafting output for plates and wires by 20%": "使用复合金属可以将金属板和线材的制作产量提高 20%",
    "Mars has the largest canyon in the solar system, known as Valles Marineris, that stretches approximately 4,000 kilometers (2,500 miles) in length, making it the largest known canyon in the solar system. To put it into perspective, Valles Marineris is more than 10 times longer and five times deeper than the Grand Canyon in the United States.": "火星拥有太阳系中最大的峡谷，称为水手谷，长约 4,000 公里（2,500 英里），是太阳系中已知最大的峡谷。 客观来看，水手谷比美国大峡谷长10倍以上，深5倍以上。",
    ", Day": "， 天",
    ", Weather:": "， 天气：",
    "Absolute Zero": "绝对零度",
    "Added \"Weather\" mechanics, available on some planets": "添加了“天气”机制，可在某些行星上使用",
    "Changed game loading mechanism to prevent data loss": "更改游戏加载机制以防止数据丢失",
    "Did you knew?": "你知道吗？",
    "Fixed some buildings prices": "修复了一些建筑物的价格",
    "Fixed/changed some goals": "修正/改变了一些目标",
    "General": "一般的",
    "General Statistics": "一般统计",
    "Implemented statistics page": "实现统计页面",
    "Increased \"Expeditions\" rewards": "增加“探险”奖励",
    "Max KP Gain (per prestige)": "最大 KP 增益（每声望）",
    "Maximum Colonists": "最大殖民者",
    "Maximum Happiness": "最大的幸福",
    "Season:": "季节：",
    "Small UI and QoL fixes": "一些小的 UI 和 QoL 修复",
    "Time In Game": "游戏时间",
    "Time This Prestige": "时间这个声望",
    "Total KP Gain (All time)": "KP 总增益（所有时间）",
    "Transmit Knowledge Prestiges": "传播知识声望",
    "Undefined": "不明确的",
    "Updated second prestige layer - fixed some formulas": "更新了第二个声望层 - 修复了一些公式",
    "v0.1.0 Big Update": "v0.1.0 大更新",
    "Year": "年",
    "The Milky Way is estimated to be home to over 100 billion stars. This vast number of stars makes the Milky Way a typical spiral galaxy, with a relatively average size compared to other galaxies in the universe.": "据估计，银河系拥有超过 1000 亿颗恒星。 如此大量的恒星使银河系成为一个典型的螺旋星系，与宇宙中的其他星系相比，其尺寸相对平均。",
    "The International Space Station completes one full orbit around the Earth every 90 minutes. As a result, its crew observes the sunrise every 90 minutes. Each day, the people on board the ISS witness 16 sunrises and 16 sunsets.": "国际空间站每 90 分钟绕地球一周。 因此，其船员每 90 分钟就会观察一次日出。 每天，国际空间站上的人们都会见证 16 次日出和 16 次日落。",
    "The first confirmed discovery of an exoplanet (a planet outside our solar system) was announced in 1992. The exoplanet, named PSR B1257+12 b, was found orbiting a pulsar, a rapidly rotating neutron star. Since then, thousands of exoplanets have been discovered using various methods, including radial velocity measurements, transit observations, and direct imaging.": "1992 年，人们首次确认发现了一颗系外行星（太阳系外的行星）。这颗系外行星被命名为 PSR B1257+12 b，被发现绕着一颗快速旋转的中子星脉冲星运行。 从那时起，人们利用各种方法发现了数千颗系外行星，包括径向速度测量、凌日观测和直接成像。",
    "Neutron stars are known for their incredibly high density. The density of a neutron star is typically around 10^17 to 10^18 kilograms per cubic meter. To put this into perspective, it is estimated that a teaspoonful of neutron star material would weigh about as much as a mountain on Earth.": "中子星以其令人难以置信的高密度而闻名。 中子星的密度通常约为每立方米 10^17 至 10^18 千克。 从这个角度来看，据估计一茶匙中子星材料的重量相当于地球上一座山的重量。",
    "Added warning on buildings running on lower efficiency": "添加了对低效率运行的建筑物的警告",
    "Knowledge: Gas & Liquid Mastery": "知识：气体和液体精通",
    "Updated formula to calculate XP bonus": "更新了计算 XP 奖励的公式",
    "v0.1.0a Small Fixes": "v0.1.0a 小修复",
    "Missing": "丢失",
    "Efficiency:": "效率：",
    "The length of blood vessels in our body is approximately 100,000 kilometers.": "我们体内血管的长度约为10万公里。",
    "Mars experiences powerful and massive dust storms that can engulf the entire planet. These storms can last for weeks or even months and cover vast regions, obscuring the Martian surface from view.": "火星经历了强大而巨大的沙尘暴，可以吞没整个星球。 这些风暴可能会持续数周甚至数月，覆盖广大地区，遮蔽火星表面。",
    "Universe is composed mostly of dark matter and dark energy, which together make up about 95% of the total energy content of the cosmos.": "宇宙主要由暗物质和暗能量组成，它们合计约占宇宙总能量的95%。",
    "Advance your knowledge about high temperature environments": "增进您对高温环境的了解",
    "Advanced Composites": "先进复合材料",
    "Advanced Evaporators": "先进的蒸发器",
    "Advanced Nuclear Research": "先进核研究",
    "Advanced Space Construction": "先进空间建设",
    "Advanced Thermodynamics": "高级热力学",
    "Allow your Biolabs to run orbital research expeditions.": "允许您的生物实验室进行轨道研究探险。",
    "Allows building spaceships for interstellar flights": "允许建造用于星际飞行的宇宙飞船",
    "Batteries Coolers": "电池冷却器",
    "Bio-labs operate normally": "生物实验室正常运行",
    "Bio-labs start consuming fuel (0.03 each) but provides 10 times more research capacity": "生物实验室开始消耗燃料（每个 0.03），但提供的研究能力增加了 10 倍",
    "Colonize New Planet": "殖民新星球",
    "Colonize Prestiges": "殖民声望",
    "Composite Cistern Membrane": "复合水箱膜",
    "Consumes huge amount of power, but provides significant research capacity": "消耗大量电力，但提供重要的研究能力",
    "Cryogenics": "低温学",
    "Each level increase rocket output by 5%": "每级增加火箭产量5%",
    "Each level provides additional space for 4 colonists": "每层都为 4 名殖民者提供额外的空间",
    "Exoplanet Science": "系外行星科学",
    "Fuel Coolers": "燃油冷却器",
    "Graphene": "石墨烯",
    "Graphene Drills": "石墨烯钻头",
    "Graphene Membranes": "石墨烯膜",
    "Hadron Collider": "强子对撞机",
    "High Energy Plasma Knife": "高能等离子刀",
    "Hold Ctrl to set 0": "按住Ctrl键设置0",
    "Hold Ctrl to set max": "按住 Ctrl 设置最大值",
    "Improved Refrigeration": "改善制冷",
    "Knowledge: Organics Mastery": "知识：有机物掌握",
    "Knowledge: Rocket Mastery": "知识：火箭掌握",
    "Law Artificial Organs: Allow": "法律人工器官：允许",
    "Learn how to combine metal with plastic to create extremely heat- and hit-hardened materials. Unlocking new upgrades and resources": "了解如何将金属与塑料结合起来，制造出耐热和耐冲击硬化的材料。 解锁新的升级和资源",
    "Learn more about building in space": "了解有关太空建筑的更多信息",
    "Learn more about elementary particles": "了解有关基本粒子的更多信息",
    "Learn more about exoplanets. Unlocks new option to search planets.": "了解有关系外行星的更多信息。 解锁搜索行星的新选项。",
    "Learn more about specific of your colonists and other organisms living in space.": "详细了解您的殖民者和生活在太空中的其他生物体的具体情况。",
    "Learn more about what matter is made of": "了解更多关于物质是由什么组成的",
    "Low Temperature Physics": "低温物理",
    "Low temperatures can significantly reduce molecule decay rates. Learn how to use it to generate more sophisticated organics": "低温可以显著降低分子衰变率。 了解如何使用它来生成更复杂的有机物",
    "Low temperatures causes some materials become incredibly good conductors. Learn more about this phenomena and how you can use it.": "低温导致某些材料成为非常好的导体。 了解有关这种现象以及如何使用它的更多信息。",
    "Magneto": "磁石发电机",
    "Magneto Cooling": "磁冷却",
    "Magneto-Dynamics": "磁动力学",
    "Max XP Gain (per prestige)": "最大 XP 增益（每声望）",
    "Nano-Biology": "纳米生物学",
    "Neutrino Physics": "中微子物理学",
    "Neutrinos are particles that interacts with other matter very weakly. Learn how to use it for your civilization development": "中微子是与其他物质相互作用非常弱的粒子。 了解如何将其用于您的文明发展",
    "Neutron Canon": "中子佳能",
    "Orbital Bio-experiments": "轨道生物实验",
    "Orbital Living Module": "轨道生活舱",
    "Orbital Supply Module": "轨道供电模块",
    "Outer Orbit Buildings": "外轨道建筑物",
    "Particle Physics": "粒子物理学",
    "Planet modifier: Cold": "行星修正：寒冷",
    "Planet modifier: Dry": "行星修正：干燥",
    "Planet modifier: Ore Poor": "行星修正：贫矿",
    "Planet modifier: Small": "行星修正：小",
    "Provides general service to your colonists": "为您的殖民者提供一般服务",
    "Quark Canon": "夸克佳能",
    "Quark Physics": "夸克物理",
    "Running extremely strong current unlocks opportunity to generate huge magnetic field. Learn more about how to maintain and use it": "运行极强的电流可以产生产生巨大磁场的机会。 了解更多关于如何维护和使用它的信息",
    "Space Biology": "太空生物学",
    "Space Warehouse": "太空仓库",
    "Spaceship Construction": "宇宙飞船建造",
    "Spaceship Factory": "宇宙飞船工厂",
    "Spaceship Station": "宇宙飞船站",
    "Super-conductivity": "超导性",
    "Thermonuclear plant": "热核电站",
    "Time This Planet": "这个星球的时间",
    "Total XP Gain (All time)": "总 XP 增益（所有时间）",
    "Universe is composed mostly of dark matter and dark energy, which together make up about 95% of the total energy content of the cosmos.": "宇宙主要由暗物质和暗能量组成，它们合计约占宇宙总能量的95%。",
    "Unlocks building ships. Each level reduce rocket consumption for all buildings by 2%": "解锁建造船只。 每个级别都会使所有建筑物的火箭消耗减少 2%",
    "Use your previous researches in biology, nanophysics and photochemistry to investigate organisms and viruses creation": "利用您之前在生物学、纳米物理学和光化学方面的研究来研究生物体和病毒的产生",
    "Using your knowledge about what protons and neutrons consists of you can learn more about nuclear manipulations": "利用您对质子和中子组成的知识，您可以了解有关核操作的更多信息",
    "UV-Polymerisation": "紫外聚合",
    "What could be better than warehouse? Yes, only warehouse in space! Also affected by storage containers.": "还有什么比仓库更好的呢？ 是的，太空中唯一的仓库！ 还受到储存容器的影响。",
    "You will lose all your resources, upgrades, buildings and science. But you will transmit your knowledge to next colonizers, making their progress much faster.": "你将失去所有的资源、升级、建筑和科学。 但你会将你的知识传递给下一个殖民者，使他们的进步更快。",
    "Venus has the longest known volcanic mountain range in our solar system, located on the surface of Venus is a massive volcanic mountain range called the \"Maxwell Montes.\" It stretches for about 1,300 kilometers (800 miles) across Venus's highland region known as Ishtar Terra.": "金星拥有太阳系中已知最长的火山山脉，位于金星表面的是一座巨大的火山山脉，称为“麦克斯韦山脉”。它绵延约 1,300 公里（800 英里），横跨金星的高地地区，称为“麦克斯韦山脉”。 伊什塔尔泰拉。",
    "Fixed issue when false warning on building efficiency dropped appeared": "修复了建筑效率下降时出现错误警告的问题",
    "Fixed layout issue on smaller screens": "修复了较小屏幕上的布局问题",
    "v0.1.0b Small Fixes": "v0.1.0b 小修复",
    "Distances between stars used to be incredibly huge. For example, the distance to Proxima Centauri from Earth is approximately 39.7 trillion kilometers or 4.24 light-years. That's why human very unlikely visit other star systems in near future. But its not the case once you are playing \"The Ignited Space\"": "恒星之间的距离曾经非常大。 例如，地球到比邻星的距离约为 39.7 万亿公里或 4.24 光年。 这就是为什么人类在不久的将来不太可能访问其他恒星系统。 但当你玩“点燃的空间”时，情况就不是这样了",
    "Fixed issue when materials with 0 capacity could be produced": "修复了可以生产 0 容量材料时的问题",
    "The human body is composed of trillions of cells, but only about 10% of those cells are actually human. Other 90% belongs to viruses, fungi, and other microscopic organisms, collectively known as the human microbiota.": "人体由数万亿个细胞组成，但其中只有约10%实际上是人类的。 其余90%属于病毒、真菌和其他微生物，统称为人类微生物群。",
    "Bulletproof polymers": "防弹聚合物",
    "Keep Expansion": "保持扩张",
    "Learn how to make plastic and rubber more durable. Decreasing rubber and plastic prices for all your buildings by 50%": "了解如何使塑料和橡胶更耐用。 将所有建筑物的橡胶和塑料价格降低 50%",
    "Use ionizing radiation for more efficient polymers generation. Improves plastic and rubber output by 10%": "使用电离辐射更有效地生成聚合物。 将塑料和橡胶产量提高 10%",
    "One fascinating fact about the universe is that it is constantly expanding. The observations made by astronomers and physicists over the past century have shown that galaxies are moving away from each other in all directions. This discovery led to the development of the Big Bang theory, which suggests that the universe originated from a hot and dense state around 13.8 billion years ago.": "关于宇宙的一个有趣的事实是它在不断膨胀。 天文学家和物理学家在过去一个世纪的观察表明，星系正在向各个方向相互远离。 这一发现导致了大爆炸理论的发展，该理论表明宇宙起源于大约 138 亿年前的炎热而致密的状态。",
    "Cold Synthesis": "低温合成",
    "Energy Superpower": "能源超级大国",
    "Graphene Synthesis": "石墨烯合成",
    "Learn how to obtain graphene from biomass": "了解如何从生物质中获取石墨烯",
    "Decreasing fuel consumption by your thermonuclear power-plant by 25%": "将热核电站的燃料消耗减少 25%",
    "Optimize High-voltage lines": "优化高压线路",
    "The Sun is not a solid object, but rather a giant, churning ball of hot plasma.": "太阳不是一个固体，而是一个巨大的、翻腾的热等离子体球。",
    "Use graphene membranes inside your batteries to boost power capacity by 10%": "在电池内部使用石墨烯膜可将电量提高 10%",
    "Your knowledge now allows you to optimize your power grid, reducing wires prices by 20% per level": "现在，您的知识可以帮助您优化电网，每级将电线价格降低 20%",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "Spring": "春天",
    "Winter": "冬天",
    "Summer": "夏天",
    "Autumn": "秋天",
    // 图标代码，不能汉化
    "Jacorb's Games": "Jacorb's Games",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "By Jacorb90": "By Jacorb90",
    "content_copy": "content_copy",
    "library_books": "library_books",
    "discord": "discord",
    "drag_handle": "drag_handle",
    "edit": "edit",
    "forum": "forum",
    "content_paste": "content_paste",
    "delete": "delete",
    "info": "info",
    "settings": "settings",

    //树游戏
    'Loading...': '加载中...',
    'ALWAYS': '一直',
    'HARD RESET': '硬重置',
    'Export to clipboard': '导出到剪切板',
    'INCOMPLETE': '不完整',
    'HIDDEN': '隐藏',
    'AUTOMATION': '自动',
    'NEVER': '从不',
    'ON': '打开',
    'OFF': '关闭',
    'SHOWN': '显示',
    'Play Again': '再次游戏',
    'Keep Going': '继续',
    'The Modding Tree Discord': '模型树Discord',
    'You have': '你有',
    'It took you {{formatTime(player.timePlayed)}} to beat the game.': '花费了 {{formatTime(player.timePlayed)}} 时间去通关游戏.',
    'Congratulations! You have reached the end and beaten this game, but for now...': '恭喜你！ 您已经结束并通关了本游戏，但就目前而言...',
    'Main Prestige Tree server': '主声望树服务器',
    'Reach {{formatWhole(ENDGAME)}} to beat the game!': '达到 {{formatWhole(ENDGAME)}} 去通关游戏!',
    "Loading... (If this takes too long it means there was a serious error!": "正在加载...（如果这花费的时间太长，则表示存在严重错误！",
    'Loading... (If this takes too long it means there was a serious error!)←': '正在加载...（如果时间太长，则表示存在严重错误！）←',
    'Main\n\t\t\t\tPrestige Tree server': '主\n\t\t\t\t声望树服务器',
    'The Modding Tree\n\t\t\t\t\t\t\tDiscord': '模型树\n\t\t\t\t\t\t\tDiscord',
    'Please check the Discord to see if there are new content updates!': '请检查 Discord 以查看是否有新的内容更新！',
    'aqua': '水色',
    'AUTOMATION, INCOMPLETE': '自动化，不完整',
    'LAST, AUTO, INCOMPLETE': '最后，自动，不完整',
    'NONE': '无',
    'P: Reset for': 'P: 重置获得',
    'Git游戏': 'Git游戏',
    'QQ群号': 'QQ群号',
    'x': 'x',
    'QQ群号:': 'QQ群号:',
    '* 启用后台游戏': '* 启用后台游戏',
    '更多同类游戏:': '更多同类游戏:',
    'I': 'I',
    'II': 'I',
    'III': 'III',
    'IV': 'IV',
    'V': 'V',
    'VI': 'VI',
    'VII': 'VII',
    'VIII': 'VIII',
    'X': 'X',
    'XI': 'XI',
    'XII': 'XII',
    'XIII': 'XIII',
    'XIV': 'XIV',
    'XV': 'XV',
    'XVI': 'XVI',
    'A': 'A',
    'B': 'B',
    'C': 'C',
    'D': 'D',
    'E': 'E',
    'F': 'F',
    'G': 'G',
    'H': 'H',
    'I': 'I',
    'J': 'J',
    'K': 'K',
    'L': 'L',
    'M': 'M',
    'N': 'N',
    'O': 'O',
    'P': 'P',
    'Q': 'Q',
    'R': 'R',
    'S': 'S',
    'T': 'T',
    'U': 'U',
    'V': 'V',
    'W': 'W',
    'X': 'X',
    'Y': 'Y',
    'Z': 'Z',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',

}


//需处理的前缀
var cnPrefix = {
    "\n": "\n",
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": " ",
    " ": " ",
    //树游戏
    "\t\t\t": "\t\t\t",
    "\n\n\t\t": "\n\n\t\t",
    "\n\t\t": "\n\t\t",
    "\t": "\t",
    "Show Milestones: ": "显示里程碑：",
    "Autosave: ": "自动保存: ",
    "Offline Prod: ": "离线生产: ",
    "Completed Challenges: ": "完成的挑战: ",
    "High-Quality Tree: ": "高质量树贴图: ",
    "Offline Time: ": "离线时间: ",
    "Theme: ": "主题: ",
    "Anti-Epilepsy Mode: ": "抗癫痫模式：",
    "In-line Exponent: ": "直列指数：",
    "Single-Tab Mode: ": "单标签模式：",
    "Time Played: ": "已玩时长：",
    "Shift-Click to Toggle Tooltips: ": "Shift-单击以切换工具提示：",
    "Resource: ": "资源：",
    "Goal: ": "目标：",
    "Allows drilling harder materials. Also improve stone output by ": "允许开采更硬的材料。 同时提高石头产量 ",
    "Improves Charge Output by ": "将电力输出提高 ",
    "Upgrade: ": "升级: ",
    "Building: ": "建筑: ",
    "Improves energy storage by ": "提升电力存储 ",
    "Happiness: ": "幸福: ",
    "Job: ": "工作: ",
    "Research: ": "研究: ",
    "Crafting: ": "制作: ",
    "You know how to review a candidate for a future colony, and how to act when things go wrong. Every point decreases healthcare requirements by ": "你知道如何审查未来殖民地的候选人，以及当出现问题时如何采取行动。 每增加一点，医疗保健要求就会减少 ",
    "Solid materials are important, but so are other states of matter. Every point boosts oxygen and water production ": "固体材料很重要，但其他物质状态也很重要。 每一点增加氧气和水的产生 ",
    "Striking the earth can be done in a wide variety of ways. Every point boosts stone, ore, iron and platinum production by ": "可以通过多种方式撞击地球。 每一点都会使石头、矿石、铁和铂的产量提高 ",
    "You are not easily bored and know how to distract people from boredom. Every point decreases entertainment requirements by ": "你不容易感到无聊，并且知道如何分散人们对无聊的注意力。 每一点减少娱乐需求 ",
    "Be more efficient in your precious biomass usage. Every point boosts biomass based creatable resources production (such as plastic and rubber) by ": "更有效地利用宝贵的生物质。 每一点都会使基于生物质的可创造资源（例如塑料和橡胶）产量提高 ",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需处理的后缀
var cnPostfix = {
    "                   ": "",
    "                  ": "",
    "                 ": "",
    "                ": "",
    "               ": "",
    "              ": "",
    "             ": "",
    "            ": "",
    "           ": "",
    "          ": "",
    "         ": "",
    "        ": "",
    "       ": "",
    "      ": "",
    "     ": "",
    "    ": "",
    "   ": "",
    "  ": "  ",
    " ": " ",
    "\n": "\n",
    "\n\t\t\t": "\n\t\t\t",
    "\t\t\n\t\t": "\t\t\n\t\t",
    "\t\t\t\t": "\t\t\t\t",
    "\n\t\t": "\n\t\t",
    "\t": "\t",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
    "": "",
}

//需排除的，正则匹配
var cnExcludeWhole = [
    /^(\d+)$/,
    /^\s*$/, //纯空格
    /^([\d\.]+):([\d\.]+)$/,
    /^([\d\.]+):([\d\.]+):([\d\.]+)$/,
    /^([\d\.]+):([\d\.]+):([\d\.]+):([\d\.]+)$/,
    /^([\d\.]+):([\d\.]+):([\d\.]+):([\d\.]+):([\d\.]+)$/,
    /^([\d\.]+)h ([\d\.]+)m ([\d\.]+)s$/,
    /^([\d\.]+)y ([\d\.]+)d ([\d\.]+)h$/,
    /^([\d\.]+)\-([\d\.]+)\-([\d\.]+)$/,
    /^([\d\.]+)e(\d+)$/,
    /^([\d\.]+)$/,
    /^\/([\d\.]+)$/,
    /^\(([\d\.]+)\)$/,
    /^([\d\.]+)\%$/,
    /^([\d\.]+)\/([\d\.]+)$/,
    /^\(([\d\.]+)\/([\d\.]+)\)$/,
    /^成本(.+)$/,
    /^\(([\d\.]+)\%\)$/,
    /^([\d\.]+):([\d\.]+):([\d\.]+)$/,
    /^([\d\.]+)K$/,
    /^\/ ([\d\.]+)K$/,
    /^([\d\.]+)M$/,
    /^([\d\.]+)B$/,
    /^([\d\.]+) K$/,
    /^([\d\.]+) M$/,
    /^([\d\.]+) B$/,
    /^([\d\.]+)s$/,
    /^([\d\.]+)x$/,
    /^x([\d\.]+)$/,
    /^([\d\.,]+)$/,
    /^\+([\d\.,]+)$/,
    /^\-([\d\.,]+)$/,
    /^([\d\.,]+)x$/,
    /^x([\d\.,]+)$/,
    /^([\d\.,]+) \/ ([\d\.,]+)$/,
    /^([\d\.]+)e([\d\.,]+)$/,
    /^([\d\.,]+)\/([\d\.]+)e([\d\.,]+)$/,
    /^([\d\.]+)e([\d\.,]+)\/([\d\.]+)e([\d\.,]+)$/,
    /^([\d\.]+)e\+([\d\.,]+)$/,
    /^e([\d\.]+)e([\d\.,]+)$/,
    /^x([\d\.]+)e([\d\.,]+)$/,
    /^([\d\.]+)e([\d\.,]+)x$/,
    /^[\u4E00-\u9FA5]+$/
];
var cnExcludePostfix = [
]

//正则替换，带数字的固定格式句子
//纯数字：(\d+)
//逗号：([\d\.,]+)
//小数点：([\d\.]+)
//原样输出的字段：(.+)
//换行加空格：\n(.+)
var cnRegReplace = new Map([
    [/^([\d\.]+) hours ([\d\.]+) minutes ([\d\.]+) seconds$/, '$1 小时 $2 分钟 $3 秒'],
    [/^You are gaining (.+) elves per second$/, '你每秒获得 $1 精灵'],
    [/^You have (.+) points$/, '你有 $1 点数'],
    [/^Transmit Knowledge Upgrades: (.+) points left$/, '传输知识升级：还剩 $1 点数'],
    [/^Next at (.+) points$/, '下一个在 $1 点数'],
	[/^([\d\.]+)\/sec$/, '$1\/秒'],
	[/^([\d\.,]+)\/sec$/, '$1\/秒'],
	[/^([\d\.,]+) OOMs\/sec$/, '$1 OOMs\/秒'],
	[/^([\d\.]+) OOMs\/sec$/, '$1 OOMs\/秒'],
	[/^([\d\.]+)e([\d\.,]+)\/sec$/, '$1e$2\/秒'],
    [/^requires ([\d\.]+) more research points$/, '需要$1个研究点'],
    [/^([\d\.]+)e([\d\.,]+) points$/, '$1e$2 点数'],
    [/^([\d\.]+) elves$/, '$1 精灵'],
    [/^([\d\.]+)d ([\d\.]+)h ([\d\.]+)m$/, '$1天 $2小时 $3分'],
    [/^([\d\.]+)e([\d\.,]+) elves$/, '$1e$2 精灵'],
    [/^([\d\.,]+) hr$/, '$1 小时'],
    [/^([\d\.,]+) min$/, '$1 分钟'],
    [/^([\d\.,]+) elves$/, '$1 精灵'],
    [/^\*(.+) to electricity gain$/, '\*$1 到电力增益'],
    [/^Cost: (.+) points$/, '成本：$1 点数'],
    [/^Req: (.+) elves$/, '要求：$1 精灵'],
    [/^Req: (.+) \/ (.+) elves$/, '要求：$1 \/ $2 精灵'],
    [/^Usages: (\d+)\/$/, '用途：$1\/'],
    [/^workers: (\d+)\/$/, '工人：$1\/'],

]);